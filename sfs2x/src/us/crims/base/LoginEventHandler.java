...
public class LoginEventHandler extends BaseServerEventHandler {
	...
	private IDBManager _dbManager = SmartFoxServer.getInstance()
			.getZoneManager().getZoneByName(Main.ZONE).getDBManager();
	// Bad username/password error
	private SFSErrorData _errData = new SFSErrorData(
	...
	public void handleServerEvent(ISFSEvent event) throws SFSException {
		...
		String encryptedPass = (String) event
		...
		String userSQL = "SELECT * FROM users WHERE username = '" + username
				+ "' LIMIT 1";
		...
		try {
			...
			throw new SFSLoginException(
					"username or password incorrect.Error1", _errData);
		}

		// At this point I would have added a conditional check to see if there
		// was a result,
		// but it is no longer needed because if no fields were retrieved, then
		// it would throw an SFSLoginException and
		...
		if (userResult.size() == 1) {
			...
			if (getApi().checkSecurePassword(session, dbPass, encryptedPass)) {
				...
				// Remove unwanted fields from the data (these don't need to be
				// sent to the client)
				 ...
				 * userInfo.putUtfString("country",
				 * userData.getUtfString("COUNTRY"));
				 * userInfo.putUtfString("mw", userData.getUtfString("MW"));
				 * userInfo.putUtfString("sw", userData.getUtfString("SW"));
				 */

				// Store userData in the current session. This is used later on
				// in ZoneEventHandler to add custom user variables
				// session.setProperty("userInfo", userInfo);

				// Obtain the response object in order to send custom userData
				// to the client upon login.
				ISFSObject responseObj = (ISFSObject) event
				...
				throw new SFSLoginException(
						"username or password incorrect.Error2", _errData);
			}
		} else {
			throw new SFSLoginException(
					"username or password incorrect.Error3", _errData);
		}
	}
