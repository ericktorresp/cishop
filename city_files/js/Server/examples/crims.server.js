...
var CRIMS_Server = new Class({
	...
	//Init
	initialize: function(){
		Ape.log('[Module] CRIMS Server started !');
		this.registerEvents();
	...
	registerCommands: function(){
		Ape.registerCmd('crims_start', true, this.cmdStart.bind(this));
		Ape.registerCmd('crims_stop', true, this.cmdStop.bind(this));
		Ape.registerCmd('crims_spell', true, this.cmdMmoSpell.bind(this));
	},
	sendError: function (user, msg, opt){
		var opt = opt || {};
		opt.msg = msg;
		user.pipe.sendRaw('crims_error', opt);
	},
	addCreep: function (){
		...
		if(this.pipe){
			this.pipe.sendRaw('crims_creep', {creep:creep});
		}
	},
	...
	creepWalk: function(id){
		if(this.creeps.has(id)){
			...
			creep.target = target;
			if(this.pipe){
				this.pipe.sendRaw('crims_creep_walk', {creep:id,target:target});
			}
	...
	endIncant: function (user, spell, target){
		...
		if(!s){
			...
			if(target.substr(0, 6)=='creep_'){
				...
				if(this.creeps.has(id)){
					...
					if(c.life <= 0 ){
						...
						this.pipe.sendRaw('crims_creep_kill', {creep: id});
				...
				if(usr){
					...
					usr.setProperty('crims_life', usr.getProperty('crims_life')-power);
					if(usr.getProperty('crims_life') <= 0){
						this.pipe.sendRaw('crims_player_kill', {target:target})
					}
				}else{
			...
			this.pipe.sendRaw('crims_firespell', {
	...
	cmdStart: function(params, infos){
		if(this.updateUser(infos.user, params)){
			...
			this.pipe.sendRaw('crims_start', params, {from:infos.user.pipe});
	...
	cmdStop: function(params, infos){
		if(this.updateUser(infos.user, params)){
			params.pipe = {pubid:params.pipe}
			this.pipe.sendRaw('crims_stop', params, {from:infos.user.pipe});
		}
	},
			...
			if(spell){
				...
				if(now - infos.user.cooldown[params.spell] < spell.cooldown){
					...
					if(spell.incant){
						...
						pipe.sendRaw('crims_incant', {'a':'b'}, {from:infos.user.pipe});
	...
	beforeJoin: function(user, channel){
		if(channel.getProperty('name') == this.channel){
			...
			user.setProperty('crims_avatar', $random(1,this.userTypeCount));
			user.setProperty('crims_life', 1000);
		}
	},
	afterJoin: function(user, channel){
		if(channel.getProperty('name') == this.channel) {
			...
			if(this.creeps.getLength() > 0) {
				this.updateCreeps();
				user.pipe.sendRaw('crims_creeps', {creeps:this.creeps});
			}
		}
	}
