...
CRIMS.MAP = new Class({
	...
	_init_map: function()
	{
		...
		console.log('init map last bottom: ',rb.bottom,'init map last right: ',rb.right);
	},
	_resize_map: function()
	{
		...
		var c=$('dragable').getElements('div')[0].getPosition(this.container);
		console.log('init left: ',c.x,'init top: ',c.y);
	},
	_make_dragable: function()
	{
		new Drag.Move($("dragable"), { 
			onDrop: function()
			{
				 ...
				 */
				var divs = $('dragable').getElements('div');
				...
				if(tl.x>0)
				{
					console.log('need load new left map!');
				}
				if(tl.y>0)
				{
					console.log('need load new top map!');
				}
				if(rb.right<c.width)
				{
					console.log('need load new right map!');
				}
				if(rb.bottom<c.height)
				{
					console.log('need load new bottom map!');
				}
			}
		});
	}
