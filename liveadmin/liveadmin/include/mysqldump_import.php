...
class MySQLDumpImport
{
	...
	function importDumpFile($dump_file)
	{
		$fs = file_get_contents($dump_file);
		$fs = str_replace('XXX-TABLE_PREFIX-XXX',LIVEADMIN_DB_PREFIX,$fs);
		$this->importDumpString($fs);
	}
	function importDumpString ($dump_str)
	{
		if ($this->dbh !== false)
		{
			$sqlFile = $dump_str;
			...
			foreach ($sqlArray as $stmt)
			{
				...
				if ($stmt != '') $sqlArrayToExecute[] = $stmt;
			}
			...
			foreach ($sqlArray as $stmt)
			{
				...
				$stmt = trim($stmt);
				$result = mysql_query($stmt,$this->dbh);
				if (!$result)
				{
					$this->Error[] = '['.mysql_errno($this->dbh).'] '.mysql_error($this->dbh);
				}
			}
		}
		else
		{
			$this->Error[] = "[0] MySQL server access denied, please check the access data login";
		}
	}
