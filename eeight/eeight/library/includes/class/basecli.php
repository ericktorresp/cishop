...
/**
 * CLI模式程序基类
 * 路径: /library/includes/class/basecli.php
 * 用途：用于动态继承其具体实现方法 _runCli() 方法
 * 
 * 使用：在实际项目里，每个CLI 项目派生于此类. 负责收集运行信息,和日志记录等操作
 * 范例:
 * 			class bonus extends basecli
 * 			{
 * 				.....
 * 				.....
 * 			}
 * 
 * 1, TODO: 获取进程运行 PID, 及运行时 CPU & 内存使用情况
 * 2, TODO: 运行记录支持文本记录及SQL记录(可供查看)
 * 3, 析构后, 需要给操作系统返回 TRUE 值
 * 4, 尽量不要太多依赖 a.php 框架中的变量与常量, 例 DIRECTORY_SEPARATOR
 * 
 * @author 	    tom  090729
 * @version	1.0.0
 * @package	core
...
abstract class basecli
{
	...
	function _initSysInfo()
	{
	    ...
	    $this->aSysInfo = array(
	        'stime' => getMicroTime(), // CLI 开始执行时间 .毫秒
	        'etime' => NULL,           // CLI 结束执行时间 .毫秒
	        'htime' => NULL,           // CLI 总执行时间   .毫秒
	        //'' => 
	    );
