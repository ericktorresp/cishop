...
/**
 * Db 数据库底层类
 * 参考 : http://cn.php.net/manual/en/class.mysqli.php
 *
 * 注意 : 此类继承于 Mysqli 类,在使用时, 请确认 php_mysqli.so|dll 的扩展是否打开
 * 
 * 
 * 依赖全局:
 *    A::$_aIni -> class.bDevelopMode            (全局开发模式), 记录显示SQL错误
 *    A::$_aIni -> class.db.bRecordProcessTime   是否记录执行 SQL 的总计时间
 *    A::$_aIni -> ... APPLE_LOGS_SQL_TO_FILE    将所有执行SQL记录至日志文件
 * 
 * 
 * 实现功能 : 
 * ~~~~~~~~~~~
 * - 允许通过全局开关 A::$_aIni -> bDevelopMode 来显示错误SQL错误信息并记录文件
 * - 允许通过全局开关 A::$_aIni -> class.db.bRecordProcessTime 记录对象执行SQL信息
 * - 对事务进行操作时, PHP程序发生异常错误或中断, 对事务进行及时回滚,避免死锁
 * - 实现 CRUD 操作 $this->insert(), $this->update(), $this->delete(), $this->getOne(), $this->getAll()
 * - 实现 getDataCached() 对 SQL 进行本地数据缓存
 * 
 * 
 * TODO : 更完善的代码
 * ~~~~~~~~~~~
 *   对所有缓存做日志记录
 *   清空所有缓存文件
 *   跟踪连接数据库时间. (建立connect连接)
 * 
 * @author    Saul,James,Tom
 * @version   1.1.0
 * @package   Core
 */
...
class db extends mysqli
{
	...
	public function __construct( $aDBO=array() ) 
	{
		...
		{
			A::halt('db.Connect Failed #1: '. $e->getMessage() .' '. mysqli_connect_error() );
		}
		if( mysqli_connect_errno() <> 0 )
		{
			 A::halt( "db.Connect Failed #2: ". mysqli_connect_error() );
			 exit;
		}
		...
		unset($sTmpHash);
		$this->bRecordProcessTime = (bool)A::getIni('class.db.bRecordProcessTime');
    ...
    private function _getDataCachedByFile( $sSql, $iTimerSec )
    {
        ...
        $mResult = $oFileCaches->readCache( $sSql, $iTimerSec );
        if( $mResult == FALSE )
        { // 1.1 缓存结果集失效
            //echo '<font color=red>11缓存结果集 失效</font>'.$sSql.'<br/>';
            $aRes = $this->getAll( $sSql );
            $oFileCaches->writeCache( $aRes, $iTimerSec );
            return $aRes;
        }
        else 
        { // 1.2 缓存结果集有效
            //echo '<font color=#669900>12缓存结果集 有效</font>'.$sSql.'<br/>';
            return $mResult;
        }
    }
