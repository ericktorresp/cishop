...
class validatecode
{
	...
	{
		if( FALSE != ($temp = A::getIni("class.validatecode.fontPath")) )
		{//使用全局字体文件路径
			$this->sFontDir = $temp;
			unset( $temp );
		}
		...
		{//默认路径
			$this->sFontDir = A_DIR . DS . 'lang' . DS . 'fonts' . DS;
		}
		$this->setCode(); 
        $this->setMolestation(); 
        $this->setImage(); 
    ...
    */ 
    ...
    { 
        ...
        {//用户指定设置
            ...
            { 
                $aFont['file'] = $this->sFontDir . 'arial.ttf'; 
            }  
        } 
        else 
        {//默认设置
