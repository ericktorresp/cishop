...
class model_snapshot extends basemodel 
{
	...
	public function doSnapshot()
	{
	    ...
	    $aRes = $this->oDB->getOne("SELECT COUNT(`entry`) AS TOMCOUNT FROM `userfund` LIMIT 1");
	    ...
	    $aTopProxyArr = $this->oDB->getAll("SELECT u.`userid`,u.`username`, "
                        ...
                        . " FROM `usertree` ut force index( idx_usertree ) LEFT JOIN `userfund_history_".$sNowDate."` uf "
	    ...
	    $aTopProxyTranferIn  = array();  // 临时数组, 总代团队的 '转入高频' (自己+下级) 仅转账成功
        $aTopProxyTranferOut = array();  // 临时数组, 总代团队的 '高频转出' (自己+下级) 仅转账成功
        $aTopProxyCancel     = array();  // 临时数组, 总代团队的 '高频转出' (自己+下级) 仅转账成功
        ...
        // ---------------------------------------------------------------------------
        // 累加转入高频(所有下级) to  $aTopProxyTranferIn
        ...
        // 累加高频转出(所有下级) to  $aTopProxyTranferOut
        ...
        for( $i=0; $i<$iTopProxyCounts; $i++ )
        {
            ...
            $sSql = " SELECT `istester`,`isfrozen` FROM `usertree` WHERE `userid` = '" . $aTopProxyArr[$i]['userid'] ."' LIMIT 1";
