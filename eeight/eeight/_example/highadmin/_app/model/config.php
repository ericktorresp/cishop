...
class model_config extends basemodel
{
	...
	function config( $iId )
	{
		...
		{
			return $this->oDB->getOne("SELECT * FROM `config` WHERE `configid`='".intval($iId)."'" . ' LIMIT 1');
		}
	}
	...
	function getConfigs( $mKeys )
	{
		...
		{
			$aConfig = $this->oDB->getOne("SELECT * FROM `config` WHERE `configkey` ='" .$mKeys. "'" . ' LIMIT 1');
		}
		if( !empty($aConfig) )
		{
		    if( is_array($mKeys) )
