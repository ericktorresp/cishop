...
class model_helps extends basemodel 
{
    ...
    {
        parent::__construct();
        if( !in_array( $sType, array( 'comment', 'faq', 'intro', 'version' )))
        { // 数据整理
            $sType = 'comment'; // 默认值 => 高频说明
        }
        $this->sType = $sType;
    }
     ...
     * @param boolean $bIsRealDel 是否真实删除
     * @return int
     */
    public function helpDel( $aHtmlCheckBox = array(), $bIsRealDel = FALSE )
    {
        $sWhere         = '';
        $sDelMessageIds = '';
        ...
        if( $bIsRealDel )
        {
            $this->oDB->query( "DELETE FROM `helpinfo` WHERE 1 $sWhere" );
        }
        else 
        {
