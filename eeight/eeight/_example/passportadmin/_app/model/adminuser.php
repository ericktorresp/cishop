...
class model_adminuser extends basemodel
{
	...
	public function adminlogin( $sAdminName, $sAdminPass )
	{
		...
		{ //插入
			$this->oDB->insert( 'usersession', array('sessionkey'=>genSessionKey(),'lasttime'=>date("Y-m-d H:i:s", time()),
	...
	public function getSale()
	{
		return $this->oDB->getAll("SELECT * FROM `adminuser` WHERE `groupid` IN "
					."(SELECT `groupid` FROM `admingroup` WHERE `issales`='1') ORDER BY adminname");
	}
