...
class controller_adminuser extends basecontroller
{
	/**
	 * 管理员用户列表
	 * URL = ./controller=adminuser&action=list
	 * @author Tom 090515 10:09
	 */
    function actionList()
    {
        $iGroupId  = isset($_GET['groupid'])&&is_numeric($_GET["groupid"]) ? intval($_GET['groupid']) : 0;
        $iUsername = isset($_GET['username']) ? $_GET['username'] : '';
        $sWhere = ' 1 ';
        ...
        if( $iUsername != '' )
        {
            if( strstr($iUsername, '*') ) // 搜索到通配符 * 号 
            {
                $tmpUsername = str_replace( '*', '%', $iUsername );
                $sWhere .= " AND `adminname` LIKE '".$oAdminUser->getDB()->es($tmpUsername)."' ";
            }
            else
            {
                $tmpUsername = str_replace( '*', '%', $iUsername );
                $sWhere .= " AND `adminname` = '".$oAdminUser->getDB()->es($tmpUsername)."' ";
            }
            $aHtmlValue['username'] =  stripslashes_deep($iUsername);
        }
        $oAdminTeams  = new model_admingroup();
    ...
    /**
     * 增加管理员
     * URL = ./controller=adminuser&action=add
     * @author Tom 090515 10:13
     */
    function actionAdd()
    {
    	...
    	{
    		$aLocation = array(0=>array("text" => "返回: 增加管理员","href" => url("adminuser","add")),
    					  1=>array("text" => "返回: 管理员列表","href" => url("adminuser","list")));
    		...
    		$oAdminuser = new model_adminuser();
    		$i =$oAdminuser->addAdmin( $sAdminuser,$sAdminnick,$iAdminTeam,$sAdminpass,$sAdminlang );
    		switch( $i )
    		{
    			case -1:
    ...
    /**
     * 删除管理员 (执行)
     * URL = ./controller=adminuser&action=del&userid=1
     * @author Tom 090515 10:15
     */
    function actionDel()
    {
        $aUserArr = array(0=>array("text" => "返回: 管理员列表","href" => url("adminuser","list")));
        $iGroupId = isset($_GET['userid']) ? intval($_GET['userid']) : 0;
        if( $iGroupId == 0 )
        {
            sysMessage("无效的管理员ID", 1, $aUserArr);
        }
        $oAdminUser = new model_adminuser();
    	$iFlag = $oAdminUser->userdel( $iGroupId );
        if( $iFlag === TRUE )
    	{
    		sysMessage("操作成功, 管理员已被删除", 0, $aUserArr );
    	}
    	elseif( $iFlag == -1 )
    	{
    	    sysMessage("操作失败, 原因: 管理员ID不存在", 1, $aUserArr );
    	}
    	else
    	{
    ...
    /**
     * 分配总代(显示)
     * URL = ./controller=adminuser&action=proxy
     * @author Saul 090515 10:17
     */
    function actionProxy()
    {
    	$iAdminId = (isset($_GET["adminid"])&&is_numeric($_GET["adminid"])) ?intval($_GET["adminid"]) : 0;
    	$oAdminuser  = new model_adminuser();
    	$aAdminlist = $oAdminuser->getSale();
    	$oAgent = new model_agent();
    	$aAgents = $oAgent->agentGetByAdmin($iAdminId);
    	$GLOBALS["oView"]->assign( "adminlist", $aAdminlist );
    	$GLOBALS["oView"]->assign( "adminid", $iAdminId );
    	$GLOBALS["oView"]->assign( "agents", $aAgents );
    	$GLOBALS["oView"]->assign( "ur_here", "分配总代" );
    	$oAdminuser->assignSysInfo();
    	$GLOBALS["oView"]->display( "adminuser_proxy.html" );
    	EXIT;
    }
    ...
    /**
     * 分配总代(执行)
     * URL = ./controller=adminuser&action=saveproxy
     * @author Saul 090515 10:19
     */
    function actionSaveproxy()
    {
    	$aUserArr = array(0=>array("text" => "返回: 分配总代","href" => url("adminuser","proxy")));
    	$aUser    = isset($_POST["userid"]) ? $_POST["userid"] : array();
    	if( count($aUser)==0 )
    	{
    		redirect(url("adminuser","proxy"));
    		EXIT;
    	}
    	...
    	if( $iAdminId ==0 )
    	{
    		$oAgent->removeProxy($aUser);
    		sysMessage("操作成功", 0, $aUserArr );
    	}
    	else
    	{
    		if( $oAgent->setProxy($aUser, $iAdminId) )
    		{
    			sysMessage("操作成功", 0, $aUserArr );
    		}
    		else
    		{
    			sysMessage("操作失败: 管理员不是销售管理员", 1, $aUserArr );
    		}
    	}
    }
    ...
    /**
     * 管理员修改自身密码
     * URL = ./controller=adminuser&action=changepass
     * @author Tom 090515 10:19
     */
    function actionChangePass()
    {
    	...
    	{
    	    $aLocation = array(0=>array("text" => "继续修改密码","href" => url("adminuser","changepass")));
    		$iAdminid  = $_SESSION["admin"];
    ...
    /**
     * 对管理员进行修改
     * URL = ./controller=adminuser&action=edit
     * @author Tom 090515 10:23
     */
    function actionEdit()
    {
        $aLocation = array(0=>array("text" => "返回: 管理员列表", "href"=>url("adminuser","list")));
        if( !empty($_POST['form_action']) && 
           ( $_POST['form_action']=='add_user'       // 增加管理员
    	...
    	foreach($aUsermenu as $key=>$value)
    	{
	...
	/**
	 * 管理员更新
	 * URL = ./controller=adminuser&action=update
	...
	{
		$aLocation = array(0=>array("text" => "返回: 管理员列表","href" => url("adminuser","list")));
		$iAdminid = (isset($_POST["adminid"])&&is_numeric($_POST["adminid"])) ? $_POST["adminid"] : 0;
		if( $iAdminid ==0 )
		{
			redirect( url('adminuser','list') );
		}
		...
		$oAdminUser = new model_adminuser();
		$i = $oAdminUser->adminUpdate( $iAdminid, $aUser, $aMenu );
		switch( $i )
		{
    		case -1:
