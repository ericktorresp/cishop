 ...
 * 支付接口 分账户信息类
 * 
 *  已知ID的支付接口分账户信息 (可传入程序用名称获取信息,但仅限于该接口下只有一个分账户)
 *  修改、添加、删除(逻辑)、日志
 *  
 * @name 	PayAccountInfo.php
 * @package payport
...
class model_pay_payaccountinfo extends model_pay_payportinfo 
{
	...
	private $BalanceTable 	 = 'payport_acc_balance';
	
	private $BalanceLogTable = 'payport_acc_balance_logs';
	/**
	...
	public function __construct($iPayAccId=null,$sKey=null){
		...
		if ( !empty($iPayAccId) ) {
			...
			if (  empty($this->Id) || empty($this->Currency) || empty($this->PayportAttr)  || empty($this->PayportName)   
				 )
				{
				  	 //关键信息不足 终止程序;
			...
			if (  ( $this->PayportAttrDrawhand != 16 )  && 
				( empty($this->ReceiveHost) || empty($this->ReceiveUrl)  )
				 )
				{
					//非人工处理方式,必须有回传域名与回调地址,否则终止程序;
					die('Error:1001 Running Stop.');
				}
	...
	public function getAccountDataObj($bReturn=false){
		...
		$this->AccSiteId 	= trim($aTmp['acc_siteid']);	
	...
	public function set(){
		...
		if ( $this->checkArrayValue($aTempData, array('acc_siteid', 'acc_mail','acc_receive_host') ) ){
			...
			if ( ($this->InputArray['acc_name'] != $this->AccName) 
				...
				if( $this->oDB->errno() > 0 ){
					...
					// 更新 充值记录分账户名 (如果需要,取消注释)
					//	$oL = new model_pay_loadinfo($this->AId);
					//	$sSql3 = $oL->redundance( array('acc_name',$this->InputArray['acc_name'],$this->AccName),'sql' );
				...
				{
					//更新分账户与用户关系表冗余
					$oUPL = new model_pay_payaccountlimit();
					$sSql2 = $oUPL->rudundance( array('pp_acc_alias',$this->InputArray['acc_name'],$this->AccName), 'sql' );
	...
	public function add(){
		...
		if (  $this->checkArrayValue($aTempData, 
			array( 'acc_siteid', 'acc_mail','srcbalance', 'inbalance', 'outbalance', 'balance','balance_limit', 'acc_receive_host', 'valid_time','isenable') 
				)  )
		{
			//开启事务
        	$this->oDB->doTransaction();
	...
	public function enable(){
		...
		else 
		{
			...
			$sSql = $oPAlimit->rudundance( array('re_pp_acc_id',$this->AId), 'sql' );
			$this->oDB->query($sSql);
			if( $this->oDB->errno() > 0 ){
				$this->oDB->doRollback();
            	return false;
	...
	public function disable(){
		...
		else
		{
			// (旧支付接口配属模式)
			$oPAlimit = new model_pay_payaccountlimit();
			$sSql = $oPAlimit->rudundance( array('save_pp_acc_id',$this->AId),'sql' );
			$this->oDB->query($sSql);
			if( $this->oDB->errno() > 0){
				$this->oDB->doRollback();
            	return false;
	...
	public function delete(){
		...
		{
			//删除分账户, 删除涉及的相关关系 (物理删除)
			$oPAlimit = new model_pay_payaccountlimit();
			$sSql = $oPAlimit->rudundance( array('del_pp_acc_id',$this->AId), 'sql' );
			$this->oDB->query($sSql);
			if ($this->oDB->errno() > 0){
				$this->oDB->doRollback();
				return false;
