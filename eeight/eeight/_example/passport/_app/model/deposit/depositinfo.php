...
class model_deposit_depositinfo extends model_pay_base_info
{
	...
	public function getPayportData($iId, $sOptType='onlineload', $iStatus='-1', $bShowIntro=false){
			...
			switch ($this->OptType){
				...
				case 'list':
					// 用于前后台列表
					$sField = 'id,payport_name,sysparam_prefix,payport_nickname,currency,load_limit_min_per,load_limit_max_per,draw_limit_min_per,draw_limit_max_per,draw_fee_step,draw_fee_per,draw_fee_percent,draw_fee_min,draw_fee_max,draw_fee_step,status,lang_code,payport_attr';
					break;
				case 'intro':
					// 用于前台显示简介
					$sField = 'id,payport_name,sysparam_prefix,payport_nickname,currency,payport_intro,status';
					break;
				default:
					// 用于后台修改
					$sField = '*';
			...
			$this->SysParamPrefix	= trim($aTmpResult['sysparam_prefix']);
	...
	public function add(){
        ...
        if ( empty($this->PayportName) ){
        	...
        	$this->oDB->doTransaction();
        	$iLastId = $this->oDB->insert( $this->TableName, $aTempData );
        	if( $this->oDB->errno() > 0 || $iLastId <= 0 ){
        		$this->oDB->doRollback();
            	return -1;
        	}
        	else{
        		// 添加 用户与银行卡绑定关系 数据表 基础数据 (初始化该银行的基本数据)
        		$oDeposit = new model_deposit_companycard();
        		$bResult = $oDeposit->copyUserbyDeposit($iLastId);
        		if ( $this->oDB->errno() > 0 || $bResult === FALSE)
        		{
        			$this->oDB->doRollback();
        			return -1;
        		}
	...
	public function erase(){
		...
		//TODO: 对用户关系表 删除的冗余 不支持, 当需要删除某个支付银行时，直接禁用其即可；
	...
	private function _setStatus($iValue){
		...
		if ( (intval($iValue) == 0) || (intval($iValue) == 1) )
		{
			...
			$sSql1 = 'UPDATE `deposit_acc_set` SET `isenable`='.intval($iAccStatus).', `utime`=\''.date('Y-m-d H:i:s').'\' WHERE `isenable`='.intval($iAccOldStatus).' AND `ads_payport_id`='.intval($this->Id);
