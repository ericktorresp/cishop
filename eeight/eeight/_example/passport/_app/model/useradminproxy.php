...
class model_useradminproxy extends basemodel 
{
    ...
    public function getAdminId( $iProxyId = 0 )
    {
        ...
        $sSql = "  SELECT `entry`,`adminid` FROM `useradminproxy` WHERE `topproxyid`='". intval($iProxyId) ."' ";
    ...
    public function getAdminSaleList( $iUserId, $sField = '', $sAndWhere = '' )
    {
        ...
        $sSql = "SELECT ".$sFields." FROM `usertree` AS ut LEFT JOIN `userchannel` AS uc ON ut.`userid`=uc.`userid` 
                ...
                AND uc.`isdisabled`='0' AND pg.`isdisabled`='0' ".$sAndWhere;
        $this->oDB->query($sSql);
        if( $this->oDB->numRows() < 1 )
        {
            return FALSE;
