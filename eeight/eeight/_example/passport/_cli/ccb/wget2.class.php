<?php
	...
	class wget2 {
		...
		public function __construct($inCharset = 'auto', $outCharset = 'utf-8', $httpVersion = '1.0', $port = 80)
        {
			$this->init();
        ...
        private function init()
        {
            ...
            $this->_responseBody = '';
			//$this->_postData = '';
            $this->_requestStream = $this->_responseStream = '';
            $this->error(0, '');
        }
		...
		public function getContents($fetchMode, $method, $url)
		{
			...
			if ($currentCharset != $this->outCharset) {
				//$this->_responseBody = iconv( $currentCharset, $this->outCharset, $this->_responseBody );
        ...
        {
            $tmpInfo = '';
            $curl = $this->addCurlHandle($method, $url, $this->_postData, $this->_requestHeaders['Cookie'], $this->_requestHeaders['Referer']);
            ...
            $this->setResponseHttpCode($info['http_code']);
            $this->setResponseHeader('Content-Type', $info['content_type']);
            curl_close($curl);
            return true;
        }
        ...
        public function getMultiContents($params)
        {
            ...
            foreach ($curls as $k => $curl) {
                ...
                if (($err = curl_error($curl)) == '') {
                    ...
                    echo "the $k occurred an error, $err\n";
                }
        ...
        public function addCurlHandle($method, $url, $postData = '', $cookie = '', $referer = '')
        {
            ...
            //curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1); //for 302
		...
		// The method must be uppercase, or it will can't transfer the post data
		public function getBySocket($method, $url)
		{
            ...
            while (!feof($this->_fp)) {
                $line = fgets($this->_fp, 8092);
				if ($line == "\r\n") {
					break;
                }
                $this->_responseStream .= $line;
			}

            $this->_responseHeaders = $this->parseResponseHeader($this->_responseStream);
            $this->_responseBody = '';
		...
		public function parseResponseHeader($header)
		{
			...
			if (!preg_match("@http/1\.\d+\s(\d{3})\s\w+@i", $statusLine, $match)) {
                $this->error(400, 'http code error');
            }
		...
		final private function connect($host)
		{
			if (!$this->_fp = fsockopen($host, $this->port, $errno, $errstr, $this->connectTimeOut)) {
