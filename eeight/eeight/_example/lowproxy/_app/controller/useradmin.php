...
class controller_useradmin extends basecontroller 
{
    ...
    function actionGroupList()
    {
        ...
        if ($oGroup->deleteMenu($iUserId) === false){
        	sysMsg( "获取组别列表失败，请联系管理员！", 2 );
        }
     ...
     * @author JAMES
     */
    /* temp_louis function actionUpdateGroup()
    {
...
// temp_louis           $aGroup   = $oGroup->getById( $iGroupid, array('groupname') );
     ...
     * @author JAMES
     */
    /* temp_louis function actionDeleteGroup()
    {
        $iUserId = intval( $_SESSION['userid'] );
        //如果为总代管理员，则当前用户调整到其总代ID
        if( $_SESSION['usertype'] == 2 )
     ...
     * @author JAMES
     */
    /* temp_louis function actionAddGroup()
    {
    ...
    function actionRightsGroup()
    {
        ...
        if( empty($_POST['flag']) || $_POST['flag']!='update' )
        {
            ...
            $aData  = $oMenus->getUserMenus( $iUserId, 1, 1 );
...
//  temp_louis          $aData  = $oGroup->getById( $iGroupid, array('menustrs','issales','viewrights','groupname') );
     ...
     * @author JAMES
     */
    /* temp_louis function actionAdminGroup()
    {
