...
class model_userfund extends basemodel 
{
	...
	public function & getProxyTeamFundList( $iUserId = 0, $sWhere = "1", $sHaving = "" )
	{
	    ...
	    { # 需求1: 指定 $iUserId 则只统计指定用户ID的所有团队资金情况
	        ...
	        $sWhere  = $sWhere.' AND ( uf.`userid` = '.intval($iUserId).' or FIND_IN_SET( '.intval($iUserId).', ut.parenttree ) ) AND ut.usertype < 2';
	        $sSql    = "SELECT $mFields FROM $sTableName WHERE $sWhere GROUP BY ut.lvtopid $sHaving ORDER BY ut.username";
	...
	public function & getProxyFundList( $iUserId = 0, $sWhere = "1" )
	{
	    ...
	    { # 需求2: 若未指定 $iUserId, 则统计所有总代的资金情况
	        $sSql   = "SELECT $mFields FROM $sTableName WHERE $sWhere AND ut.`parentid` = 0  ";
	        $aResult = $this->oDB->getAll($sSql);
	        return $aResult;
	    }
	}
	...
	public function apiFundTransition( $aDatas = array() )
	{
		...
		$sSql = "SELECT `username` FROM `usertree` WHERE `userid`='".$aDatas['iUserId']."' AND `isdeleted`='0' ";
