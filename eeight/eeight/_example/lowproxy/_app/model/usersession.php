...
class model_usersession extends basemodel 
{
	...
	public function checkMenuAccess( $iUserId = 0, $sControlName = '', $sActionName = '' )
	{
        ...
        if( $iUserId > 0 )
        {
           ...
           $aResult = $this->oDB->getDataCached( $sSql, 60 );
           if( empty($aResult) )
           {
              return -3;
           }
...
//            }
		...
		$sSql  = "SELECT `title`,`menuid`,`actionlog`,`ismenu`,`islink`,`islabel` FROM `usermenu` " 
				  ...
				  . "' AND `actioner`='" . daddslashes($sActionName) . "' AND `isdisabled`='0' AND `menutype`='1' ";
		...
		if ($_SESSION['usertype'] == 2){
			$sSql = " SELECT ut.`userid`,ut.`username`,ut.`usertype`,uc.`groupid`,uc.`extendmenustr` 
		          FROM `userchannel` AS uc LEFT JOIN `usertree` AS ut ON ut.`userid`=uc.`userid`
		          WHERE ut.`isdeleted`='0' AND ut.`userid`='".$iUserId."' AND uc.channelid = " . SYS_CHANNELID;
		} else {
			$sSql = " SELECT ut.`userid`,ut.`username`,ut.`usertype`,ugs.`groupid`,ugs.`extendmenustr` 
		          FROM `usergroupset` AS ugs LEFT JOIN `usertree` AS ut ON ut.`userid`=ugs.`userid`
