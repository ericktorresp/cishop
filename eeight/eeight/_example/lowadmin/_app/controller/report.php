 ...
 * + actionFundlist         游戏币明细表
 * + actionSnapshot         查看低频快照报表
...
class controller_report extends basecontroller
{
    ...
    function actionOrderlist()
    {
        ...
        $aSearch['method']     = isset($_GET["method"])&&is_numeric($_GET["method"]) ? intval($_GET["method"]) : 0;
        ...
        { // 进行数据整理(更新订单号), 对当页数据进行小结
            ...
            {
                $v['orderno'] = model_orders::orderEnCode('O'.date("Ymd").'-'.$v['entry'], "ENCODE");
                if( $v['signamount'] == 0 )
                {
                ...
                if($v["projectid"]>0)
                {
                    $v["projectid"] = model_projects::ProjectEnCode('D'.$v["issue"]."-".$v["projectid"],"ENCODE");
                }
            }
        }
        ...
        $aMethod          =  $oMethod->methodGetList('a.`methodid`,a.`methodname`,a.`lotteryid`,b.`cnname`',"a.`pid`>0",'',0);
        $aLottery         = array();
        $methods          = array();
        foreach ($aMethod as $method)
        {
            $aLottery[$method["lotteryid"]] = $method["cnname"];
            if(!isset($methods[$method["lotteryid"]]))
            {
                $methods[$method["lotteryid"]]="<select name='method' id='method'><option value='0'>所有玩法</option>";
            }
            $methods[$method["lotteryid"]] = $methods[$method["lotteryid"]]."<option value='".$method["methodid"];
            if($method["methodid"] == $aSearch["method"])
            {
                $methods[$method["lotteryid"]] = $methods[$method["lotteryid"]]." selected='selected'";
            }
            $methods[$method["lotteryid"]] = $methods[$method["lotteryid"]]."'>".$method["methodname"]."</option>";
        }
        $aHtmlValue['lottery'] = $aLottery;
        $aHtmlValue['method']  = json_encode($methods);
        /* @var $oPassPort model_passport */
        //$oPassPort        = A::singleton("model_passport");
        $oPassPort = new model_passport($GLOBALS['aSysDbServer']['report']);
    ...
    function actionSaleList()
    {
        ...
        //$oMethod = A::singleton("model_method");
        $oMethod = new model_method($GLOBALS['aSysDbServer']['report']);
        $aMethod = $oMethod->methodGetList("a.`methodid`,a.`methodname`,b.`cnname`,b.`lotteryid`",'a.`pid`>0','',0);
        $aLottery = array();
        $aMethods = array();
        foreach( $aMethod as $vMethod )
        {
            $aLottery[$vMethod["lotteryid"]] = $vMethod["cnname"];
            $aMethods[$vMethod["lotteryid"]][$vMethod["methodid"]] = $vMethod["methodname"];
        }
        $sWhere = "";
        $sWhere1 = "";
        ...
        //$oUser = A::singleton("model_user");
        $oUser = new model_user($GLOBALS['aSysDbServer']['report']);
        $aUser = $oUser->getChildList(0,'','','',0); //代理获取
        $GLOBALS['oView']->assign( "proxy", $aUser["results"]);
        $iUserId = 0;
        ...
        if( $starttime!="" )
        {
            ...
            $sWhere1 .= " AND o.`actiontime`>='".$starttime."'";
        ...
        if( $endtime!="" )
        {
            ...
            $sWhere1 .= " AND o.`actiontime`<='".$endtime."'";
        }
        ...
        { //自身部分这一行
            $Self = array();
            $aSelf = $oOrders->getUserBonusAndPrize($iUserId,$sWhere, true, $sWhere1);
            if( !empty($aSelf) )
            {
                $self["userid"]   = $iUserId;
        ...
        $sUserWhere = "";
        //测试账户
        if( isset($_GET['istester']) && $_GET['istester'] != -1 && $iUserId == 0 )
        {
            $sUserWhere .= " AND a.`istester` = " . intval($_GET['istester']);
        }
        //冻结账户
        ...
        $aResult = $oOrders->getTotalBonusAndPrize( $iUserId, $sWhere, $sUserWhere, true, $sWhere1 );
    ...
    function actionSaledetail()
    {
        ...
        /* @var $oOrders model_orders */
        $oOrders = A::singleton("model_orders");
        $aResult = $oOrders->getTotalUserBonusByMethod($iUserId, $sWhere, FALSE, $sWhere1);
        if( empty($aResult) )
        {
    ...
    // 01, 整理搜索条件
        ...
        //$oOrder = A::singleton("model_orders");
        $oOrder = new model_orders($GLOBALS['aSysDbServer']['report']);
        $aResult= $oOrder->getTopProxyTransition( $aSearch['sdate'], $aSearch['edate'], $sWhere, $sTranStatusSql );
    ...
    function actionSnapshot()
    {
        ...
        $oBankSnapshot = new model_snapshot($GLOBALS['aSysDbServer']['report']);
	    ...
	    //$oBankSnapshot = A::singleton('model_orders');
            $oBankSnapshot = new model_orders($GLOBALS['aSysDbServer']['report']);
        ...
        $GLOBALS['oView']->assign( 'ur_here', '低频快照报表' );
    ...
    function actionSinglesale()
    {
        ...
        $oSale = new model_sale($GLOBALS['aSysDbServer']['report']);
        ...
        $aSearch['starttime']       = isset($_GET['starttime']) ? $_GET['starttime'] : '';
        $aSearch['endtime']         = isset($_GET['endtime']) ? $_GET['endtime'] : '';
        $pn                         = isset($_GET['pn'])? intval($_GET['pn']) : 25;// 分页用1
        $iPn 	= $GLOBALS['SysPageSizeMax'] > 1 && $pn > $GLOBALS['SysPageSizeMax'] 
        ...
        {
            $sCondition .= " AND sl.`joindate` >= '" . $aSearch['starttime'] . "'";
        }
        if( $aSearch['endtime'] )//统计的结束时间
        {
            $sCondition .= " AND sl.`joindate` <= '" . $aSearch['endtime'] . "'";
        }
        $aSaleList = $oSale->getSingleSale( '', $sCondition, '', $iPn, $p );
        $aSale = array();
        //定义合计值
        $fTotalPrice  = 0;
        $fTotalCharge  = 0;
        ...
        foreach ( $aSaleList['results'] as $aValue )
        {
            if( $aSearch['includetestuser'] )//包含测试账户
            {
                ...
                $aValue['charge'] += $aValue['test_charge'];
                $aValue['bonus']  += $aValue['test_bonus'];
                $aValue['return'] += $aValue['test_return'];
            }
            $aValue['saleresult'] = $aValue['sell'] - $aValue['bonus'] - $aValue['return'];//单期盈亏值
            $aSale[] = $aValue;
            //合计值
            $fTotalPrice  += $aValue['sell'];
            $fTotalCharge += $aValue['charge'];
        ...
        $GLOBALS['oView']->assign( 'totalcharge',  $fTotalCharge);
