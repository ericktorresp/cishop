...
class controller_admingroup extends basecontroller
{
    /**
     * 组别列表
     * URL = ./index.php?controller=admingroup&action=list
     * @author Tom
     */
    function actionList()
    {
    ...
    /**
     * 修改组别 (前台)
     * URL = ./index.php?controller=admingroup&action=edit
     * @author Tom 
     */
    function actionEdit()
    {
        $aLocation[0] = array("text" => "返回组别列表", "href" => url("admingroup","list") );
        $iGroupId     = is_numeric($_GET["groupid"]) ? intval($_GET["groupid"]) : 0;
        if( $iGroupId==0 )
        {
        ...
        /* @var $oAdminGroup model_admingroup */
        $oAdminGroup = A::singleton('model_admingroup');
        $aAdminGroup = $oAdminGroup->admingroup( $iGroupId ); // 获取组别所有权限菜单 [menustrs]
    ...
    /**
     * 修改组别 (执行)
     * URL = ./index.php?controller=admingroup&action=update
     * @author Tom
     */
    function actionUpdate()
    {
        $aLocation[0] = array("text" => "返回管理员分组列表","href" => url("admingroup","list"));
        if( !isset($_POST) )
        {
           ...
           ))
        {
        	/* @var $oAdminGroup model_admingroup */
            $oAdminGroup = A::singleton('model_admingroup');
            if( $_POST['form_action'] == 'bat_enableall' ) // 组别批量启用
            {
        ...
        /* @var $oAdminGroup model_admingroup */
        $oAdminGroup = A::singleton('model_admingroup');
        $iGroupId = isset($_POST["groupid"])&&is_numeric($_POST["groupid"]) ? intval($_POST["groupid"]) : 0;
        $aAdminGroup = $oAdminGroup->admingroup( $iGroupId );
        if( $aAdminGroup == -1 )
        {
            sysMessage( "无效的组别ID", 1, $aLocation );
    ...
    /**
     * 复制组别 (前台)
     * URL = ./index.php?controller=admingroup&action=copy&groupid=1
     * @author Tom
     */
    function actionCopy()
    {
    ...
    /**
     * 删除组别(执行)
     * URL = ./index.php?controller=admingroup&action=del
     * @author Tom
     */
    function actionDel()
    {
        ...
        /* @var $oAdminGroup model_admingroup */
        $oAdminGroup = A::singleton('model_admingroup');
        $iFlag = $oAdminGroup->delete( $iGroupId );
        if( $iFlag === TRUE )
        {
            sysMessage("操作成功", 0, $aLocation );
        }
