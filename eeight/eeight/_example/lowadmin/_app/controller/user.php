...
class controller_user extends basecontroller
{
    ...
    function actionUserlist()
    {
        ...
        $pn = isset($_GET['pn']) ? intval($_GET['pn']) : 20;   // 分页用2
        $searchpn = $GLOBALS['SysPageSizeMax'] > 1 && $pn > $GLOBALS['SysPageSizeMax'] 
						? $GLOBALS['SysPageSizeMax'] 
						: $pn = $pn > 5 ? $pn : 20; 
		
        $aUsers  = $oUser->getChildrenList($userid, $sFileds, $sWhere, $sOrderBy, $searchpn, $iPage, $bool, TRUE, 0);
        $GLOBALS['oView']->assign("users",$aUsers["results"]);
        if(($aUsers['affects']==0)&&(!empty($aUsers['self'])))
        { //用户个数修正..
            $aUsers['affects'] = 1;
        }
        $oPage = new pages( $aUsers['affects'], $searchpn );
        ...
        $oUser->assignSysInfo();
        $aUserTree = $oUser->getParent($userid, TRUE);
        $GLOBALS['oView']->assign("ur_here","用户列表");
        $GLOBALS['oView']->assign("searchpn", $searchpn);
        $GLOBALS['oView']->assign("banner", $aUserTree);
    ...
    {
        $do = isset($_GET["do"])&&!empty($_GET["do"]) ? daddslashes($_GET["do"]):"index";
        if( $do=="index" )
        {
        ...
        elseif( $do=="menu" )
        {
            ...
            { //返回JSON
                $oUser = new model_user($GLOBALS['aSysDbServer']['report']);
                $aUser = $oUser->getChildListID( $id, " ORDER BY a.username", TRUE );
    ...
    function actionSetprize()
    {
        ...
        $aUser = $oUser->userAgentget("1 ORDER BY B.username");
        $GLOBALS['oView']->assign("aUser",$aUser);
        /* @var $oLottery model_lottery */
        $oLottery     = A::singleton("model_lottery");
        $aLottery     = $oLottery->lotteryGetList( "`cnname`,`lotteryid`", "", "", 0 );
