...
class model_prizegroup extends basemodel
{
	...
	function pgInsert( $aOldPrizeGroup )
	{
		...
		$aLottery = $this->oDB->getOne("select * from `lottery` WHERE `lotteryid`='".$iLotteryid."'");
		if(empty($aLottery))
		{ //彩种不存在
			return -2;
		}
		...
		$aPrizeGroup["status"]	 = 1; //内容需要审核
		$aMothod = $this->oDB->getAll("SELECT `methodid`,`level`,`nocount`,`totalmoney` FROM `method`"
				." where `lotteryid`='".$iLotteryid."' and `pid`='0'");
		foreach($aMothod as $method)
		{ //检查方案设置
	...
	function pgGetOne($sFilds, $sCondition )
	{
		...
		return $this->oDB->getOne("SELECT ".$sFilds." FROM `prizegroup` WHERE ".$sCondition);
	}
	...
	function pgUpdate( $aOldPrizeGroup, $iPrizeGroupid )
	{
		...
		foreach($aMethod as $method)
		{
			for($i=1;$i<=$method['level'];$i++)
			{
				...
				." `methodid`='".$method["methodid"]."' AND `level`='".$i."'";
	...
	function userAuth($iPrizegroup,$aUser)
	{
		...
		{
			//需要保留原始数据
			//$this->oDB->query("Update `prizegroup` set `status`=(3&`status`) where `prizegroupid`='".$iPrizegroup."' and `topproxy`!=''");
			//$this->oDB->query("Update `prizegroup` set `status`=(1&`status`) where `prizegroupid`='".$iPrizegroup."' and `topproxy`=''");
	...
	function pgVerifity($iPrizeGroupid)
	{
		...
		$aPrizeGroup = $this->oDB->getOne("SELECT * FROM `prizegroup` WHERE `prizegroupid`='".$iPrizeGroupid."'");
		if(empty($aPrizeGroup))
		{ //奖组不存在
			return -2;
		}
		...
		foreach($users as $user)
		{
			...
			{
				$aUserPrize = $this->oDB->getOne("SELECT * FROM `userprizegroup` WHERE `userid`='".$user."' and `pgid`='".$iPrizeGroupid."'");
				if(empty($aUserPrize))
				{//新加用户的需要插入
					//用户奖组信息表
				...
				{ //用户的需要更新
					...
					foreach($aPrizeLevel as $prizelevel)
					{
						$aUserOldPL = $this->oDB->getOne("SELECT * FROM `userprizelevel` where `userpgid`='".$aUserPrize['userpgid']."' and `plid`='".$prizelevel['prizeid']."'");
						if(empty($aUserOldPL))
						{ //历史奖组不存在，需要插入
						    $aUserPrizeLevel = array();
						...
						{ //有历史记录的需要更新记录
							...
							if($prizelevel['userpoint']<$aUserOldPL['userpoint'])
							{
								...
								."`prizegroupid`='".$aUserOldPL['userpgid']."' order by `userpoint` DESC limit 0,1";
