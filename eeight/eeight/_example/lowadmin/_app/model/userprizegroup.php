...
class model_userprizegroup extends basemodel
{
	...
	function userpgGetList($sFields='', $sCondition='', $sOrderBy='', $iPageRecord=0, $iCurrentPage=0)
	{
		...
		{
			return $this->oDB->getPageResult('userprizegroup', $sFields, $sCondition, $iPageRecord,
	...
	function userpgGetOne( $sField, $sCondition ="1" )
	{
		...
		return $this->oDB->getOne("SELECT  ".$sField." FROM `userprizegroup` WHERE ".$sCondition);
	}
	...
	function userpgUpdate( $aNewUserPrizeGroup, $iUserPrizeGroupid )
	{
		...
		$sSql = "SELECT * FROM `userprizegroup` where `userpgid`='".$iUserPrizeGroupid."'";
		...
		//获取对应模板
		$sSql = "SELECT * FROM `prizegroup` where `prizegroupid`='".$aOldUserGroup['pgid']."'";
		...
		$sSql = "SELECT `minprofit`,`mincommissiongap` FROM `lottery` WHERE `lotteryid`='".$aOldUserGroup['lotteryid']."'";
		...
		foreach($aUserPrizeLevel as $i=>$v)
		{
			...
			$aUpdate["description"] = isset( $aNewUserPrizeGroup["description"][$v["methodid"]] ) 
				? $aNewUserPrizeGroup["description"][$v["methodid"]][$v["level"]]
				: "";
			...
			{ // 如果玩法没有同步给总代
				if(isset($aOldUserPrize[$v["methodid"]]))
				{
					...
					{
						$aCheck = $this->oDB->getOne("SELECT `userpoint` FROM `usermethodset` where `methodid`='".$v["methodid"]."' and `prizegroupid`='".$iUserPrizeGroupid."' order by `userpoint` DESC limit 0,1");
	...
	function userpgVerifity($pgid)
	{
		...
		if(!empty($pgid))
		{
			$this->oDB->query("update `userprizegroup` SET `status`='1' where `userpgid` in (".join(",",$pgid).")");
			if($this->oDB->errno()>0)
			{
				return -1;
			}
	...
	function userPrizegroupstop($iPgid)
	{
		...
		$aUserPrize = $this->oDB->getOne("SELECT * FROM `userprizegroup` where `userpgid`='".$iPgid."'");
		if(empty($aUserPrize))
		{ //用户奖组没有找到
			return -1;
		}
	 ...
	 * 对奖金同步状态进行检查
	 *
	 * @param unknown_type $iPrizeGroup
	 * @return unknown
	 */
