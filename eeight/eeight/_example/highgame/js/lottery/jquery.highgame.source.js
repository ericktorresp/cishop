...
;(function($){//start
    ...
    $.gameInit = function(opts){//整个购彩界面的初始化
        ...
        $.each(opts.data_label, function(i,n){//生成标签
            if(typeof(n)=='object'){
                if( i == 0 ){//第一个标签自动选择
                    bhtml += '<li class="selected" value="'+i+'"><a href="javascript:">'+n.title+'</a></li>';
                    lt_smalllabel({//生成该标签下的小标签
                            title:n.title,
                            label:n.label });
                }else{
                    bhtml += '<li value="'+i+'"><a href="javascript:">'+n.title+'</a></li>';
                }
            }
        });
        $bhtml = $(bhtml);
        ...
        $($.lt_id_data.id_labelbox).children().click(function(){//切换标签
            if( $.trim($(this).attr("class")) == 'selected' ){//如果已经是当前标签则不切换
                return;
            }
            $($.lt_id_data.id_labelbox).children().removeClass("selected");
    ...
    $.fn.lt_selectarea = function( opts ){
        ...
        if( otype == 'input' ){//输入型，则载入输入型的数据
            ...
            html += '<tr><td><textarea id="lt_write_box"></textarea><br />'+tempdes+'</td><td class="writebutbox"><img src="'+opts.imagedir+'botton_delmul.jpg" id="lt_write_del" /><br /><img src="'+opts.imagedir+'botton_import.jpg" id="lt_write_import" /><br /><img src="'+opts.imagedir+'botton_empty.jpg" id="lt_write_empty" /></td></tr>';
            data_sel[0] = []; //初始数据
            tempdes = null;
            ...
            $.each(opts.layout, function(i,n){
                if(typeof(n)=='object'){
                    ...
                    html += '<td><div class="selbuttonbox">';
                    numbers = n.no.split("|");
                    ...
                    if( opts.isButton == true ){
                        html += '<div class="selfixbutton" name="all">'+lot_lang.bt_sel_all+'</div><div class="selfixbutton" name="big">'+lot_lang.bt_sel_big+'</div><div class="selfixbutton" name="small">'+lot_lang.bt_sel_small+'</div><div class="selfixbutton" name="odd">'+lot_lang.bt_sel_odd+'</div><div class="selfixbutton" name="even">'+lot_lang.bt_sel_even+'</div><div class="selcleanbutton" name="clean">'+lot_lang.bt_sel_clean+'</div>';
                    }
                    html += '<td></tr>';
                }
            });
            ...
            $.each(opts.layout, function(i,n){
                ...
                html += '<td><div class="seldxdsbox">';
                numbers = n.no.split("|");
                for( i=0; i<numbers.length; i++ ){
                    html += '<div name="lt_place_'+n.place+'">'+numbers[i]+'</div>';
                }
                html += '</div><div class="selcleanbutton" name="clean">'+lot_lang.bt_sel_clean+'</div><td></tr>';
            });
        }else if( otype == 'dds' ){
            $.each(opts.layout, function(i,n){
                n.place  = parseInt(n.place,10);
        ...
        if( opts.isButton == true ){//如果有这几个按钮才做处理
            $("div[class='selfixbutton']",$(this)).click(function(){//清除处理
        ...
        $($.lt_id_data.id_sel_insert).unbind("click").click(function(){
            ...
            var cfhtml = '<tr><th>'+nohtml+'</th><td>'+$.lt_method_data.modes[modes].name+'</td><td>'+nums+lot_lang.dec_s1+'</td><td>'+times+lot_lang.dec_s2+'</td><td>'+money+lot_lang.dec_s3+'</td><td class="del"><span>X</span><input type="hidden" name="lt_project[]" value="'+serverdata+'" /></td></tr>';
            var $cfhtml = $(cfhtml);
    ...
    $.fn.lt_trace = function(){
        ...
        var t_count = $.lt_issues.today.length+$.lt_issues.tomorrow.length;//可追号期数
        var t_nowpos= 0;//当前起始期在追号列表的位置[超过该位置的就不在处理,优化等待时间]
        //装载可追号期数
        $($.lt_id_data.id_tra_alct).html(t_count);
        //装载同倍、翻倍标签
        var htmllabel = '<span class="selected" id="lt_margin">'+lot_lang.dec_s13+'</span><span id="lt_sametime">'+lot_lang.dec_s10+'</span><span id="lt_difftime">'+lot_lang.dec_s11+'</span>';//<span id="profit">'+lot_lang.dec_s12+'</span>
        var htmltext  = '<span id="lt_margin_html">'+lot_lang.dec_s14+'<input name="lt_trace_times_margin" type="text" id="lt_trace_times_margin" value="1" />&nbsp;&nbsp;'+lot_lang.dec_s29+'<input name="lt_trace_margin" type="text" id="lt_trace_margin" value="50" />%&nbsp;&nbsp;</span>';
        htmltext += '<span id="lt_sametime_html" style="display:none;">'+lot_lang.dec_s14+'<input name="lt_trace_times_same" type="text" id="lt_trace_times_same" value="1" /></span>';
        htmltext += '<span id="lt_difftime_html" style="display:none;">'+lot_lang.dec_s17+'<input name="lt_trace_diff" type="text" id="lt_trace_diff" value="1" />'+lot_lang.dec_s18+'&nbsp;&nbsp;'+lot_lang.dec_s2+' '+lot_lang.dec_s19+' <input name="lt_trace_times_diff" type="text" id="lt_trace_times_diff" value="2" /></span>';
        htmltext += '&nbsp;&nbsp;'+lot_lang.dec_s15+':<input name="lt_trace_count_input" type="text" id="lt_trace_count_input" style="width:36px" value="10" /><input type="hidden" id="lt_trace_money" name="lt_trace_money" value="0" />';
        $(htmllabel).appendTo($.lt_id_data.id_tra_label);
        $(htmltext).appendTo($.lt_id_data.id_tra_lhtml);
        $('#lt_margin').click(function(){//利润率
            if( $(this).attr("class") != "selected" ){
                $(this).attr("class","selected");
                $('#lt_sametime').attr("class","");
                $('#lt_difftime').attr("class","");
                $('#lt_margin_html').show();
        ...
        //装载追号的期号列表
        var issueshtml = '<table cellpadding="0" cellspacing="0" id="lt_trace_issues_today">';
        $.each($.lt_issues.today,function(i,n){
            issueshtml += '<tr id="tr_trace_'+n.issue+'"><td class="checkid"><input type="checkbox" name="lt_trace_issues[]" value="'+n.issue+'" /></td><td class="issue">'+n.issue+'</td><td class="inputtimes"><input name="lt_trace_times_'+n.issue+'" type="text" class="textbutton" value="0" disabled/>'+lot_lang.dec_s2+'</td><td class="rlmoney">'+lot_lang.dec_s20+'<span id="lt_trace_money_'+n.issue+'">0.00</span></td><td>'+n.endtime+'</td></tr>';
        });
        issueshtml += '</table><table cellpadding="0" cellspacing="0" id="lt_trace_issues_tom" style="display:none;">';
        ...
        $($.lt_id_data.id_tra_ok).click(function(){
            ...
            $.confirm(msg,function(){
                ...
                for( i=0; i<ic; i++ ){
                    ...
                    $($s[i]).children().addClass("selected");
                }
                $.lt_trace_issue = ic;
                $.lt_trace_money = tm;
                upTraceCount();
    ...
    $.fn.lt_timer = function(start,end){//服务器开始时间，服务器结束时间
        ...
        var timerno = window.setInterval(function(){
            if($.lt_time_leave > 0 && ($.lt_time_leave % firstTime == 0 || $.lt_time_leave == secondTime )){//分开用户的请求时间，在3分30秒至4分29秒的时间段内;30秒至1分29秒的时间段内随机读取服务器时间
                $.ajax({
                        type: 'POST',
                        URL : $.lt_ajaxurl,
                        timeout : 30000,
