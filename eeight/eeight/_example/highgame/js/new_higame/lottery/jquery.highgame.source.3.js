...
;(function($){//start
    ...
    $.gameInit = function(opts){//整个购彩界面的初始化
        ...
        $.extend({
            ...
            lt_method : {371:'BJRX1',373:'BJRX2',375:'BJRX3',377:'BJRX4',379:'BJRX5',381:'BJRX6',383:'BJRX7',385:'BJHZDS',387:'BJHZDX',389:'BJSXP',391:'BJJOP'},
    ...
    var lt_smalllabel = function(opts){//动态载入小标签
        ...
        $.each(opts.label, function(i,n){
            if(typeof(n)=='object'){
                if( i > 0 && i % 4 == 0 && $.lt_lottid != 9 ){//4个小标签一换行(除BJKL8)
                    html += '</div><div>';
                }
                if( i == 0 ){//第一个标签自动选择
					if($.lt_lottid == 9)
					{
						var methodname = $.lt_method[n.methodid];//玩法的简写,如:'ZX3'
						switch(methodname)
						{
							case 'BJRX1' :
								html += '<span class="methodgroupname">普通玩法：</span>';
							break;
							case 'BJHZDS' :
								html += '<span class="methodgroupname">和值：</span>';
							break;
						}
					}
					...
					if($.lt_lottid == 9)
					{
						...
						{
							case 'BJSXP' :
								html += '&nbsp;&nbsp;&nbsp;&nbsp;<span class="methodgroupname">盘面：</span>';
							break;
						}
					}
        ...
        $("input[name='smalllabel']").click(function(){
            ...
            $.lt_method_data = {
								...
								maxcodecount: opts.label[index].maxcodecount
                              };
    ...
    $.fn.lt_selectarea = function( opts ){
        ...
        var html  = '<div id="right_05"><table cellpadding="0" cellspacing="0" width="99%">';
        ...
        if( otype == 'input' ){//输入型，则载入输入型的数据
            ...
            $.each(opts.layout, function(i,n){
                if(typeof(n)=='object'){
                    ...
                    html += '';
                    if(opts.title.length > 0 && i == 0){
                        html += '<tr><td colspan="'+n.cols+'" class="td_title">';
                        html += '<div class="'+(opts.title.length<3 ? 'two' : (opts.title.length>3 ? 'four' : 'three') )+'"><div>'+opts.title+'</div></div>';
                        if( opts.isButton == true ){
                            if( $.lt_lottid == 9 )
                            {
                                html += '<div class="right_02_11"><div name="clean" class="selcleanbutton_bj">'+lot_lang.bt_sel_clean+'</div></div>';
                            }
                            html += '</td></tr>';
                        }
                    }
                    ...
                    if(n.cols > 1 && n.title.length > 0){
                        html += '<td rowspan="'+n.cols+'" class="tagtitle">';
                        html += '<div class="buttontag"><div>'+n.title+'</div></div>';
                        html += '</td>';
                        html += '<td class="mainnumber"><div class="right_02_11">';
                    }else{
                        html += '<td class="mainnumbernotitle"><div class="right_02_11">';
                    }
                    numbers = n.no.split("|");
                    for( j=0; j<numbers.length; j++ ){
                        if(numbers[j] == 'sp'){
                            ...
                            html += '<div name="lt_place_'+n.place+'" class="button1">'+numbers[j]+'</div>';
                        }
                    }
                    html += '</div></td></tr>';
                    if( opts.isButton == true ){
						if( $.lt_lottid != 9 )
						{
							html += '<div class="right_02_11"><div name="all" class="button3">'+lot_lang.bt_sel_all+'</div><div class="button3" name="big">'+lot_lang.bt_sel_big+'</div><div class="button3" name="small">'+lot_lang.bt_sel_small+'</div><div class="button3" name="odd">'+lot_lang.bt_sel_odd+'</div><div class="button3" name="even">'+lot_lang.bt_sel_even+'</div><div class="selcleanbutton" name="clean">'+lot_lang.bt_sel_clean+'</div></div>';
						}
						html += '</td></tr>';
                    }
                }
                ...
                if(typeof(n)=='object'){
                    ...
                    if( n.cols > 0 ){//有标题
                        if( n.title.length > 0 ){
                            ...
                            html += '<div class="'+(n.title.length<3 ? 'two' : (n.title.length>3 ? 'four' : 'three') )+'">'+n.title+'</div>';
                            html += '</td>';
                        }
                    }
                    html += '<td><div class="right_02_11 bjkl8">';
                    numbers = n.no.split("|");
                    for( i=0; i<numbers.length; i++ )
                    {
                        html += '<div name="lt_place_'+n.place+'" class="button14">';
                        html += numbers[i];
                        html += '</div>'
                    }
                    if( opts.isButton == true ){
        ...
        var _SortNum = function(a,b){//数字大小排序
            if( otype != 'input' ){
                a = a.replace(/大/g,0).replace(/小/g,1).replace(/单/g,0).replace(/双/g,1).replace(/奇/g,0).replace(/偶/g,1).replace(/上/g,0).replace(/下/g,1).replace(/中/g,2).replace(/和/g,2).replace(/\s/g,"");
                b = b.replace(/大/g,0).replace(/小/g,1).replace(/单/g,0).replace(/双/g,1).replace(/奇/g,0).replace(/偶/g,1).replace(/上/g,0).replace(/下/g,1).replace(/中/g,2).replace(/和/g,2).replace(/\s/g,"");
            }
            a = parseInt(a,10);
            b = parseInt(b,10);
            if( isNaN(a) || isNaN(b) ){
        ...
        function selectNum( obj, isButton ){
            if( $(obj).hasClass('selected') ){//如果本身是已选中，则不做任何处理
                return;
            }
            $(obj).addClass("selected");//样式改变为选中
            place = Number($(obj).attr("name").replace("lt_place_",""));
            var number = $.trim($(obj).html().toLowerCase());
        ...
        function unSelectNum( obj, isButton ){
            if( !$(obj).hasClass('selected') ){//如果本身是未选中，则不做任何处理
                return;
            }
            $(obj).removeClass("selected");//样式改变为未选中
            place = Number($(obj).attr("name").replace("lt_place_",""));
            var number = $.trim($(obj).html().toLowerCase());
            number = number.replace(/\<div.*\<\/div>/g,"").replace(/\r\n/g,"");
        ...
        function changeNoCss(obj){
            if( $(obj).hasClass('selected') ){//如果本身是已选中，则做取消
                unSelectNum(obj,false);
            }else{
                selectNum(obj,false);
            }
        ...
        if( opts.isButton == true ){//如果有这几个按钮才做处理
            ...
            $("div[class='selcleanbutton_bj']",$(this)).click(function(){//清除处理
				$.each($('.selected',$('#right_05')), function(i,n){
					unSelectNum(n,true);
				});
				checkNum();
            });
    ...
    $.fn.lt_trace = function(){
        var t_type  = 'same';//追号形式[利润率:margin,同倍:same,翻倍:diff]
        $.extend({
        ...
        var htmllabel = '';
       /* htmllabel += '<div id="button111">'+lot_lang.dec_s13'</div>';*/
		htmllabel += '<div id="button121">'+lot_lang.dec_s10+'</div><div id="button13">'+lot_lang.dec_s11+'</div>';
        var htmltext  = '';
		htmltext  += '<td align="left">追号计划：&nbsp;'
		//htmltext  += '<span id="lt_margin_html">'+lot_lang.dec_s14+'&nbsp;<input class="input02" name="lt_trace_times_margin" type="text" id="lt_trace_times_margin" value="1" />&nbsp;'+lot_lang.dec_s29+'&nbsp;<input class="input02" name="lt_trace_margin" type="text" id="lt_trace_margin" value="50" />&nbsp;%&nbsp;</span>';
        htmltext += '<span id="lt_sametime_html">'+lot_lang.dec_s14+'&nbsp;<input name="lt_trace_times_same" type="text" class="input022" id="lt_trace_times_same" value="1" /></span>';
