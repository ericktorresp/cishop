...
class model_proxygroup extends basemodel 
{
    ...
    public function isPermitProxy( $mGroupId, $iUserId )
    {
        ...
        $mResult = $this->getById( $mGroupId, array('ownerid') );
        if( empty($mResult) )
        {
            return FALSE;
        }
	...
	public function getById( $iGroupId, $aGroupInfo=array() )
	{
		...
		$sSql .= " FROM `proxygroup` WHERE `groupid`='". intval($iGroupId) ."' LIMIT 1";
	...
	public function & getListByUser( $iUserId )
	{
		...
		//清理admin_proxy_menu，保证数据完成性
		$sSql = 'DELETE FROM admin_proxy_menu WHERE groupid NOT IN (SELECT groupid FROM proxygroup WHERE ownerid='.intval($iUserId).' OR ownerid=0) AND ownerid='.intval($iUserId);
		$this->oDB->query($sSql);
	...
	public function updateByUser( $iUserId, $iGroupId, $aGroupInfo )
	{
		...
		//首先判断是否为默认组
		$temp_aData = $this->getById( $iGroupId, array('isspecial','ownerid','menustrs','groupname','isdisabled','issales') );
		if( empty($temp_aData) )
		{
			return FALSE;
		}
		...
		if( $temp_aData['isspecial'] == 0 )
		{
			...
			//调用API写入到 passport.proxygroup
			//2010/5/25 Floyd
        	$oChannelApi = new channelapi( 0, 'syncAdminProxyMenu', TRUE );
        	$oChannelApi->setTimeOut(10);            // 设置读取超时时间
            $oChannelApi->setResultType('serial');   // 设置返回数据类型 json | serial
			$aParams = array(
				"userid"=>$iUserId,
				"groupid"=>$iGroupId, 
				'groupname'=>$temp_aData['groupname'],
				'issales'=>$temp_aData['issales']
			);
            $oChannelApi->sendRequest( $aParams );    // 发送结果集
            $aResult = $oChannelApi->getDatas();
            if( empty($aResult) || !is_array($aResult) || $aResult['status'] == 'error' )
            {//调用API激活失败
            	return FALSE;
            }
			$iProxyGroupId = $aResult['data'];
			$oAdminProxyMenu = A::singleton('model_adminproxymenu');
			$bResult = $oAdminProxyMenu->insert( array('groupid'=>$iProxyGroupId,'menustrs'=>$aGroupInfo['menustrs'],'ownerid'=>$iUserId) );
			if( !$bResult )
			{//插入新权限失败
				$this->oDB->doRollback();
				return FALSE;
			}
			//调用API写入到其他频道.admin_proxy_menu
        	$oChannelApi = new channelapi( 1, 'syncAdminProxyMenu', TRUE );
        	$oChannelApi->setTimeOut(10);            // 设置读取超时时间
            $oChannelApi->setResultType('serial');   // 设置返回数据类型 json | serial
			$aParams = array(
				'groupid'=>$iGroupId, 
				'newgroupid'=>$iProxyGroupId,
				'userid'=>$iUserId
			);
            $oChannelApi->sendRequest( $aParams );    // 发送结果集
            $aResult = $oChannelApi->getDatas();
		...
		{//不为默认组而且为自己的组则直接修改
			return $this->oDB->update( 'admin_proxy_menu', array('menustrs'=>$aGroupInfo['menustrs']), 			                           " `groupid`=".$iGroupId );
		}
		return TRUE;
	}
     ...
     * 
     * @access  public
     * @since 	2010-08-26			copy from passport
     * @author  louis
