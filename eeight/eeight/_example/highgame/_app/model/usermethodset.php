...
class model_usermethodset extends basemodel 
{
    ...
    public function & userMethodSetGetOne( $sFields='', $sCondition='' )
    {
    	...
    	$sSql       = "SELECT ".$sFields." FROM `usermethodset` ".$sCondition." LIMIT 1"; 
	 ...
	 * @author Floyd
	 * @param int $iUserId
	 * @param array $aIndefiniteIds
	 * @param boolean $bInclude
	 *
	 * @return array
	 */
    public function getUserMinPoint( $iUserId, $aIndefiniteIds, $bInclude=FALSE, $bIsTop=FALSE )
	{
		if(!$iUserId || !$aIndefiniteIds)
		{
			return FALSE;
		}
        if( $bIsTop == TRUE )
        {//如果为总代则结合总代奖金组设置表
        	$sSql = "SELECT l.`mincommissiongap`,upg.`userpgid`,upg.`title`,upg.`lotteryid`,MIN(upl.`userpoint`) as point FROM `userprizegroup` AS upg LEFT JOIN `userprizelevel` AS upl ON upg.`userpgid`=upl.`userpgid` LEFT JOIN `method` AS m ON upl.`methodid`=m.`methodid` LEFT JOIN lottery AS l ON l.`lotteryid`=upg.`lotteryid` WHERE upg.`status`='1' AND upg.`userid`='".$iUserId."' AND upl.`isclose`='0' AND m.`isclose`='0' ";
			if(!$bInclude)
			{
				$sSql .= ' AND m.`methodid` NOT IN ('.implode(',',$aIndefiniteIds).') ';
			}
			else
			{
				$sSql .= ' AND m.`methodid` IN ('.implode(',',$aIndefiniteIds).') ';
			}
		}
        else 
        {//普通代理为结合用户玩法设置表
			$sSql = "SELECT l.`mincommissiongap`,upg.`userpgid`,upg.`title`,upg.`lotteryid`,MIN(ums.`userpoint`) as point FROM `userprizegroup` AS upg LEFT JOIN `usermethodset` AS ums ON upg.`userpgid`=ums.`prizegroupid` LEFT JOIN lottery AS l ON l.`lotteryid`=upg.`lotteryid` WHERE upg.`status`='1' AND ums.`userid`='$iUserId' AND ums.`isclose`='0' ";
			if(!$bInclude)
			{
				$sSql .= ' AND ums.`methodid` NOT IN ('.implode(',',$aIndefiniteIds).') ';
			}
			else
			{
				$sSql .= ' AND ums.`methodid` IN ('.implode(',',$aIndefiniteIds).') ';
			}
		}
		$sSql .= 'GROUP BY upg.`userpgid`';
		return $this->oDB->getAll($sSql);
	}
     ...
     * @return  array    //结果集
     */
    public function & getUserChildMaxPoint( $iUserId, $aIndefiniteIds, $bInclude )
    {
    	$aResult = array();
        if( empty($iUserId) || !is_numeric($iUserId) || $iUserId <= 0 )
        {//用户信息错误
        ...
        $iUserId = intval($iUserId);
        $sSql = "SELECT MAX(ums.`userpoint`) AS maxuserpoint,ums.`prizegroupid` FROM `usermethodset` AS ums ";
        $sSql .= "LEFT JOIN `usertree` AS ut ON ums.`userid`=ut.`userid` ";
        $sSql .= "WHERE ut.`parentid`='".$iUserId."' AND ut.`isdeleted`='0' AND ums.`isclose`='0' ";
		if(!$bInclude)
		{
			$sSql .= ' AND ums.`methodid` NOT IN ('.implode(',',$aIndefiniteIds).')';
		}
		...
		$sSql .= "group by ums.`prizegroupid`";
        return $this->oDB->getAll($sSql);
    }
     ...
     */
    public function rapidUserPoint( $iUserId, $aArr=array() )
    {
    	if( empty($iUserId) || !is_numeric($iUserId) || $iUserId <= 0 )
        {//用户信息错误
            return FALSE;
        ...
        {//检测数据安全和完整性
        	foreach( $aArr as &$v )
        	{
				if( empty($v['oid']) || !is_numeric($v['oid']) || intval($v['oid']) <= 0 )
        		{//操作用户错误
        			return FALSE;
        		}
				if( empty($v['lotteryid']) || !is_numeric($v['lotteryid']) || intval($v['lotteryid']) <= 0 )
        		{//彩种设置错误
        			return FALSE;
        		}
        	    ...
        	    if( isset($v['userindefinitepoint']) && (!is_numeric($v['userindefinitepoint']) || $v['userindefinitepoint'] < 0) )
                {//不定位返点设置错误
                    return FALSE;
                }
                $v['userpoint']    = round(floatval($v['userpoint']), 3);
                $v['userindefinitepoint']    = round(floatval($v['userindefinitepoint']), 3);
                $v['limitbonus']   = (isset($v['limitbonus']) && intval($v['limitbonus'])>=0) ? $v['limitbonus'] : 0;
                $v['isclose']      = 0; //修改的都为开启
        	}
			//1. 读取操作用户各彩种已开通的玩法 (method)
            $oUserPrize  = A::singleton("model_userprizegroup");
            $aUserPrizes = $oUserPrize->getUserPrizeGroupList( $aArr[0]['oid'], $aArr[0]['oIsTop'], "DISTINCT m.methodid, upg.userpgid,upg.`lotteryid`" );
			$aLottery = array();
			foreach($aUserPrizes as $up)
			{
				$aLottery[$up['lotteryid']][] = $up['methodid'];
			}
			//2. 循环写入/更新 usermethodset
			foreach($aArr as $a)
			{
				$aMethodIds = array();
				foreach($aLottery[$a['lotteryid']] as $m)
				{
					$aMethodIds[] = array(
					   'userid'       => $iUserId,
					   'methodid'     => $m,
					   'prizegroupid' => $a['prizegroupid'],
					   'userpoint'    => in_array($m,$this->aIndefiniteIds) ? $a['userindefinitepoint'] : $a['userpoint'],
					   'isclose'      => 0
					);
				}
				$this->setUserPoint( $iUserId, $v['lotteryid'], $aMethodIds );
			}
        }
        return TRUE;
    }
    ...
    public function setUserPoint( $iUserId, $iLotteryId, $aArr=array() )
    {
        ...
        $sSql = " SELECT upg.`userpgid`,l.`mincommissiongap` FROM `usermethodset` AS ums 
                  ...
                  WHERE ums.`userid`='".$iUserId."' AND upg.`lotteryid`='".$iLotteryId."' LIMIT 1";
        ...
        {//如果没有设置过并且有数据可以设置，则检测用户是否激活
            ...
            {//如果没有数据则插入
                ...
                //判断是否为一代
                $sSql = " SELECT `usertype`,`lvproxyid` FROM `usertree` WHERE `userid`='".$iUserId."' LIMIT 1";
            ...
            if( $iFlagUF <= 0 )
            {//如果没有数据则插入
                $aTempData = array('userid'=>$iUserId, 'channelid'=>SYS_CHANNELID, 'lastupdatetime'=>date("Y-m-d H:i:s"), 'lastactivetime'=>date("Y-m-d H:i:s") );
                //插入数据
        ...
        foreach( $aArr as &$vv )
        {
        	...
        	$sSql = " SELECT `entry`,`userpoint`,`prizegroupid` FROM `usermethodset` 
        	           WHERE `userid`='".$iUserId."' AND `methodid`='".$vv['methodid']."' LIMIT 1";
        	$aResult = $this->oDB->getOne($sSql);
        	if( $this->oDB->errno() > 0 )
        	{//数据库操作错误
        		$this->oDB->doRollback();
        	...
        	{//有则修改
        		...
        		{//如果更改后的返点小于原来的返点则检测被修改用户下级的该玩法的最大返点
        			$sSql = " SELECT MAX(ums.`userpoint`) AS usermaxpoint FROM `usermethodset` AS ums
        			          ...
        			          AND `methodid`='".$vv['methodid']."' LIMIT 1";
    ...
    public function gdSetUserPoint( $iUserId )
    {
        ...
        $sSql = " SELECT `usertype`,`lvtopid`,`lvproxyid`,`parenttree`,`parentid` 
                  FROM `usertree` WHERE `userid`='".$iUserId."' AND `isdeleted`='0' LIMIT 1";
        ...
        $sSql = "SELECT count(`entry`) AS allcount FROM `usermethodset` WHERE `userid`='".$aSelfData['parentid']."' LIMIT 1";
        ...
        //检查用户自己的奖金组情况
        $sSql      = "SELECT count(`entry`) AS allcount FROM `usermethodset` WHERE `userid`='".$iUserId."' LIMIT 1";
        $aTempData = $this->oDB->getOne( $sSql );
        if( $this->oDB->errno() > 0 )
        {
        	return -4;
     ...
     */
    public function & getParentPoint( $iUserId, $sFields='', $sCondition='' )
    {
    	$aResult = array();
        if( empty($iUserId) || !is_numeric($iUserId) || $iUserId <= 0 )
        {//用户信息错误
        ...
        $sFields   = empty($sFields) ? 
                     'ut.`userid`,ut.`lvtopid`,ut.`lvproxyid`,ums.`userpoint`,ums.`isclose`,ums.`methodid`' : 
                     daddslashes($sFields);
        $sSql      = " SELECT ".$sFields."
                       FROM `usertree` AS ut
                       LEFT JOIN `usermethodset` AS ums ON ut.`userid`=ums.`userid` 
                       WHERE ut.`isdeleted`='0' AND ut.`userid` IN(".$sParents.") 
                       AND ut.`parentid`<>'0' ".$sCondition;
