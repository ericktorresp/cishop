...
class model_user extends basemodel
{
	...
	public function getParentId( $iUserId, $bAllParentId=FALSE )
	{
		...
		{//获取所有上级
			$sSql = "SELECT `parenttree` AS `parents` FROM `usertree` 
						WHERE `userid`='".$iUserId."' AND `isdeleted`='0' LIMIT 1";
		}
		else 
		{
	...
	public function getTopProxyId( $iUserId, $bUserInfo=FALSE )
	{
		...
		$sSql = "SELECT `userid`,`username` FROM `usertree` WHERE `userid`='".$iPrentId.
	...
	public function getAdminList( $iUserId, $sOrderby='' )
	{
		...
		$sSql = "SELECT ut.`userid`,ut.`nickname`,ut.`username`,pg.`groupid`,pg.`groupname`,
				...
				FROM `usertree` AS ut 
				LEFT JOIN `userchannel` AS ugs ON (ut.`userid`=ugs.`userid` AND ugs.`channelid`=".SYS_CHANNELID.") 
				LEFT JOIN `proxygroup` AS pg ON ugs.`groupid`=pg.`groupid` 
				WHERE 
				ut.`parentid`='".intval($iUserId)."' AND ut.`isdeleted`='0' AND ut.`usertype`='2' ".$sOrderby;
		return $this->oDB->getAll( $sSql );
	}
	...
	public function getUserExtentdInfo( $iUserId, $iUserType=0 )
	{
		...
		if( $iUserType == 2 )
		{
			$sSql = "SELECT ut.`userid`,ut.`username`,ut.`nickname`,g.`groupname`,g.`groupid`,
					...
					WHERE ut.`isdeleted`='0' AND ut.`userid`='".$iUserId."' LIMIT 1";
		}
		else 
		{
			$sSql = "SELECT ut.`userid`,ut.`username`,ut.`nickname`,g.`groupname`,g.`groupid`,
	...
	public function & getUserLeftInfo( $iUserId, $iUserType=1, $sFields="" )
	{
        ...
        if( $aResult['parentid'] > 0 )
        {
            $sSql = " SELECT `userrank` FROM `usertree` WHERE `isdeleted`='0' 
                      AND `userid`='".$aResult['parentid']."' LIMIT 1";
	...
	public function getActiveUserCount( $sDate = '' )
	{
	    ...
	    $aResult = $this->oDB->getOne( "SELECT count(`userid`) AS TOMCOUNT from `users` WHERE `lasttime` >= '$sDate' LIMIT 1" );
	...
	{
	    $aResult = $this->oDB->getOne( "SELECT count(`userid`) AS TOMCOUNT from `userchannel` where ".
	                                   "`isdisabled`='0' and `channelid`='".SYS_CHANNELID."' LIMIT 1" );
	    return ($this->oDB->ar()>0) ? $aResult['TOMCOUNT'] : 0;
	}
	...
	function getUnderZeroUser( $aUserId = array() )
	{
	    ...
	    {
	        $sWhere = ' uf.`channelbalance`<0 OR uf.`holdbalance`<0 OR uf.`availablebalance`<0 OR (uf.`cashbalance`<0 AND ut.`parentid`!=0) ';
	    }
