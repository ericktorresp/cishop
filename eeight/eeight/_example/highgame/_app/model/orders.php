...
class model_orders extends basemodel
{
	...
	function addOrders( $aOrders = array() )
	{
	    ...
	    $iPackageId  = isset($aOrders['iPackageId']) ? intval($aOrders['iPackageId']) : 0; // 定单ID
	    ...
	    $iModesId    = isset($aOrders['iModesId']) ? intval($aOrders['iModesId']) : 0; // 模式ID
	    ...
	    $aRes = $this->oDB->getOne("SELECT * FROM `userfund` WHERE `userid`='$iFromUserId' AND `channelid`=".SYS_CHANNELID." LIMIT 1");
	    ...
	    $aOrderDatas['packageid']       =   $iPackageId;
	    $aOrderDatas['taskid']          =   $iTaskId;
	    $aOrderDatas['projectid']       =   $iProjectId;
	    $aOrderDatas['modes']           =   $iModesId;
	...
	public function UpdateApiTranferStatus( $iOrderEntry=0, $sUniqueKey='', $iRelationOrderEntry=0, $iFlag=2, $iAdminId=0, $sAdminName='' ) 
	{
		...
		$aUpdate['actiontime']      = date("Y-m-d H:i:s");
     ...
     * @param <type> $iPageRecords
     * @param <type> $iCurrPage
     * @return <type>
     * @author Rojer
     */
    public function getAdminOrderList2( $sFields = "*" , $sCondition = "1", $iPageRecords = 25 , $iCurrPage = 1)
	{
		$sTableName = "`orders` AS o ".
		              " LEFT JOIN `usertree` AS ut ON ut.`userid`=o.`fromuserid`".
					  " LEFT JOIN `projects` AS P ON (o.`projectid`=P.`projectid`) ";
		$sFields = "ut.`userid`,ut.`username`,o.`entry`,o.`title`,o.`amount`,o.`preavailable`,o.`availablebalance`,
		o.`entry`,o.`projectid`,o.`description`,o.`uniquekey`,o.`ordertypeid`,o.`modes`,P.`issue`, ".
				   " o.`times`,o.`transferstatus`, o.`adminname`,o.`clientip`,o.`lotteryid`,o.`methodid`,P.`codetype` ";
		$sCountTableName = "`orders` AS o LEFT JOIN `usertree` AS ut ON ut.`userid`=o.`fromuserid` ";
		$sCountSql = " SELECT COUNT(*) AS TOMCOUNT FROM ".$sCountTableName." WHERE ".$sCondition;
		return $this->oDB->getPageResult( $sTableName, $sFields, $sCondition, $iPageRecords, $iCurrPage,' ORDER BY o.`entry` DESC ', '', $sCountSql );
	}
	...
	public function getOrderType( $sReturnType = 'arr', $mSelected='', $sAndWhere='' )
	{
		...
		if( $sReturnType == 'arr' )
		{
		    return self::array_spec_key($aReturn, 'id');
		}
	...
	function getOrdersTotal()
	{
		$aResult = $this->oDB->getOne(" SELECT COUNT(`entry`) AS `TOMCOUNT` FROM `orders` WHERE 
		                                DATE(`actiontime`)='".date("Y-m-d")."' LIMIT 1");
	...
	function getFundTotalByUserId( $iUserId )
	{
		...
		$aProjects = $oProjects->projectGetResult( $iUserId, FALSE, 
		              ...
		              " AND P.`iscancel`=0 AND P.`writetime` >= '".date('Y-m-d 02:20:00')."'  ", '', 0 );
		$aUser["totalin"] = $aProjects[0]["TOMSUM"];
		unset($oProjects,$aProjects);
		// 获取 '奖金派送' 金额
		$aGet = $this->oDB->getDataCached(" SELECT SUM(`bonus`) AS sum_amount FROM `projects` WHERE 
                                            `userid` = '".$iUserId."' AND `iscancel` = 0 AND `prizestatus`=1 ".
                                            " AND `bonustime` >=  '".date('Y-m-d 02:20:00')."' ", 10);
		/*$aGet = $this->oDB->getDataCached(" SELECT SUM(amount) AS sum_amount FROM `orders` WHERE 
		                                    `fromuserid` = '".$iUserId."' AND `ordertypeid` = '".
		                                     ORDER_TYPE_JJPS."' AND `times` >=  '".date('Y-m-d 02:20:00')."' ", 10);*/
	...
	function userOrderList( $iUserid, $bAllChild, $sField, $sWhere, $sOrder, $iPageRecord, $iCurrPage = 1 )
	{
		...
		if( empty( $sField ) )
		{
			$sField = "O.*,`UT`.`username`,P.`issue`,P.`codetype`";
		}
		$sCondition = " 1 ";
		if( !empty( $sWhere ) )
		{
		...
		$sCountTableName = "`orders` AS O";
		if( $bAllChild || strpos($sCondition, "UT.") !== FALSE)
		{
		    //同时查询用户自己和下级时，关联用户树表,指定用户,不包含下级时，不用关联用户树表
		    $sCountTableName .= " LEFT JOIN `usertree` AS UT ON (O.`fromuserid`=UT.`userid`) ";
		}
		if( strpos($sCondition, "P.") !== FALSE  )
		{
		    //统计账变总数时在需要按奖期查询时关联方案表,没有指定奖期进行查询时，不用关联方案表
		    $sCountTableName .= " LEFT JOIN `projects` AS P on (O.`projectid`=P.`projectid`)" ;
		}
		$sCountSql = " SELECT COUNT(*) AS TOMCOUNT FROM ".$sCountTableName." WHERE ".$sCondition;
		return $this->oDB->getPageResult( $sTableName, $sField, $sCondition, $iPageRecord, $iCurrPage, $sOrder, '', $sCountSql );
	}
	...
	function getProxyTotalPoint( $iProxyId = 0, $sCondition = '' )
	{
		...
		$aReturn = $this->oDB->getOne( " SELECT SUM(`amount`) AS totalpoint FROM `orders` FORCE INDEX ( idx_search ) 
		                             WHERE $sReturnCondition LIMIT 1" );
		...
		$aResultCancel = $this->oDB->getOne( " SELECT SUM(`amount`) AS totalpoint FROM `orders` FORCE INDEX ( idx_search ) 
		                             WHERE $sCancelCondition LIMIT 1" );
	 ...
	 * @return array
	 */
	function getTotalBonusAndPrize( $iUserId, $sCondition, $sUserCondition )
	{
		$iUserId = intval($iUserId);
		if( $iUserId == 0 )
		{ // 查询总代的
			...
			$aTemp      = $this->oDB->getDataCached( " SELECT ".$sField." FROM ".$sTableName." WHERE 1 ".$sWhere." 
			                                    GROUP BY P.`lvtopid` ", 300 );
			$aResult    = array();
			...
			$sField = "SUM(o.`amount`) AS amount,P.`lvtopid`";
			$sWhere = " AND o.`ordertypeid` = '$iOrderTypeId'".$sCondition.$sUserCondition;

			$aTempAmout  = $this->oDB->getDataCached( " SELECT ".$sField." FROM ".$sTableName." WHERE 1 ".$sWhere." 
			                                    GROUP BY P.`lvtopid` ", 300 );
		...
		{ // 查询非总代的
			$aUserInfo  = $this->oDB->getOne("SELECT * FROM `usertree` WHERE `userid`='".$iUserId."' LIMIT 1");
			...
			$aTemp1     = $this->oDB->getDataCached(" SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhere.
			                                  $sUserCondition." AND P.`iscancel`='0' group by P.`userid`", 300 );	
			$aResult    = array();
			foreach( $aTemp1 as $v)
			{				
			...
			/*计算大额撤单手续费*/
			$iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
			$sTableName = " `orders` AS o JOIN `usertree` AS a LEFT JOIN `projects` as P ON (P.`projectid` = o.`projectid`) ";
			...
			$sWhere     = " AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." a.`userid` = P.`userid` AND o.`ordertypeid` = '$iOrderTypeId' ".$sCondition;
			$aTemp1     = $this->oDB->getDataCached(" SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhere.
			                                  $sUserCondition." GROUP BY o.`fromuserid`", 300 );
			foreach( $aTemp1 as $v)
			{				
				$a = explode( ",", $v["USERTREE"] );
				if( $v["parentid"] == $iUserId )
			...
			$aTemp      = $this->oDB->getDataCached("SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhere.
			               $sUserCondition." AND UDP.`status`=1 AND P.`iscancel`='0' GROUP BY UDP.`userid`", 300 );
	 ...
	 * @author mark
	 */
	function getUserBonusAndPrize( $iUserId, $sCondition )
	{
		$aResult = array();
		$iUserId = intval($iUserId);
		if( $iUserId <= 0 )
		...
		//查询用户是否总代
		$sUseSql = " SELECT * FROM `usertree` WHERE `userid` = '" . $iUserId . "' LIMIT 1";
		$aUser = $this->oDB->getOne($sUseSql);
		if( empty($aUser) )
		{
		    return $aResult;
		}
		elseif ($iUserId == $aUser['lvtopid'])//总代：统计返点
		{
		    $sSql = " SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		          FROM `usertree` AS UT,`userdiffpoints` AS UDP
		          LEFT JOIN `projects` AS P ON (P.`projectid`=UDP.`projectid`) 
		          WHERE 1 " . $sCondition. " AND P.`iscancel`='0' AND UDP.`userid`='".$iUserId."' AND UT.`userid`='".$iUserId."' 
		          GROUP BY P.`modes`";
		    $aResult = $this->oDB->getDataCached( $sSql, 300 );
		}
		else //非总代
		{
		    $sSql = " SELECT SUM(P.`bonus`) AS `bonus`,SUM(P.`totalprice`) AS `price`,P.`modes`,
		          UT.`username`,UT.`usertype` FROM `usertree` AS UT LEFT JOIN `projects` AS P ON 
		          (P.`userid`=UT.`userid`) WHERE 1 "
		    .$sCondition." AND P.`iscancel`='0' AND P.`userid`='".$iUserId."' GROUP BY P.`modes`";
		    $iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
		    $aResult = $this->oDB->getDataCached( $sSql, 300 );
		    //自身返点
		    $sSql = " SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		              ...
		              WHERE 1 " . $sCondition. " AND P.`iscancel`='0' AND UDP.`userid`='".$iUserId."' AND UT.`userid`='".$iUserId."' 
		              GROUP BY P.`modes`";
		    $aPointResult = $this->oDB->getDataCached( $sSql, 300 );
		    //计算大额撤单手续费
		    $sSql = "SELECT SUM(`amount`) AS amount ,P.`modes` FROM `orders` AS o LEFT JOIN `projects` AS P ON( o.`projectid` = P.`projectid` )
		         WHERE `fromuserid` = '$iUserId' AND o.`ordertypeid` = '$iOrderTypeId'" . $sCondition ." GROUP BY P.`modes`";
		    $aPrice = $this->oDB->getDataCached( $sSql, 300 );//获取大额撤单手续费
		}
		$aLasResult = array();
		$aLasResult['bonus']  = 0.00;
		$aLasResult['point']  = 0.00;
		$aLasResult['price']  = 0.00;
		$aLasResult['usertype'] = $aUser['usertype'];
		$aLasResult['username'] = $aUser['username'];
		//按元角模式分组、遍历元角模式数据
		foreach ( $aResult as $aValue )
		{
		    $aLasResult['bonus'] += isset($aValue['bonus']) ? $aValue['bonus'] : 0;
		    $aLasResult['point'] += isset($aValue['point']) ? $aValue['point'] : 0;
		    $aLasResult['price'] += isset($aValue['price']) ? $aValue['price'] : 0;
		}
		...
		if( isset($aPrice) )
		{
		    ...
		    {
		        $aLasResult['price'] += $aValue['amount'];
		    }
		}
		return $aLasResult;
	}
	 ...
	 * @return array
	 */
	function getTotalUserBonusByMethod( $iUserId, $sCondition, $needCheck = TRUE )
	{
		if( !isset($needCheck) )
		{
			$needCheck = TRUE;
		...
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(UDP.`diffmoney`) AS `summoney`,P.`modes`"
		...
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(P.`bonus`) AS `sumbonus`,SUM(P.`totalprice`) AS `sumprice`,P.`modes`"
   ...
   /**
    ...
    public function getOrderList( $sFiled = '*', $sCondition = '1', $bGetOne = TRUE )
    {
        if( $bGetOne )
        {
            return $this->oDB->getOne( " SELECT $sFiled FROM `orders` WHERE $sCondition LIMIT 1" );
        }
        else
        {
     ...
     * @author mark
     */
    public function getReportData( $iUserId = 0, $sCondition = 'AND 1', $bIsChild = FALSE )
    {
        $iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
        $iUserId = intval($iUserId);
        if ( $iUserId == 0 )
        ...
        {//查询用户下级
            ...
            $aResult = $this->oDB->getDataCached( $sSql, 7200 );
            /*归类直接下级*/
            foreach ( $aResult as $aValue )
            {
            ...
            $sSql .= $sCondition;
            $sSql .= " GROUP BY p.`userid` ";
            $aCancelAmount = $this->oDB->getDataCached( $sSql, 7200 );
            /*归类直接下级*/
            ...
            $aPointResult = $this->oDB->getDataCached( $sSql, 7200 );
            /*归类直接下级*/
            foreach ( $aPointResult as $aValue )
            {
    ...
    /**
	 * 获取后台报表的成本和奖金以及返点总和(结果集已经整理,通过 UDP来计算的)
	 * @author mark
	 * @param integer $iUserId
	 * @param string $sCondition
	 * @return array
	 */
	function getAdminTotalBonusAndPrize( $iUserId, $sCondition, $sUserCondition, $sQueryStartTime = '', $sQueryEndTime = '' )
	{
		$iUserId = intval($iUserId);
		if( $iUserId == 0 )
		{ // 查询总代的
		    $sTableName = " `projects` AS P JOIN `usertree` AS a ";
            //获取总代购费
			$sPrizeField = " SUM(P.`totalprice`) AS `SUM_PRIZE`,P.`lvtopid` ";
			$sPrizeConditon = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    {
		       $sPrizeConditon .= " AND P.`deducttime`>= '" . $sQueryStartTime ."'";  
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		    {
		        $sPrizeConditon .= " AND P.`deducttime`<= '" . $sQueryEndTime ."'";
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		    {
		        $sPrizeConditon .= " AND (P.`deducttime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		    }
			$sPrizeWhere = " AND P.`iscancel` = '0' AND a.`userid`=P.`userid` " . $sCondition . $sPrizeConditon . $sUserCondition;	
			$aPrizeTemp = $this->oDB->getDataCached( " SELECT ".$sPrizeField." FROM ".$sTableName." WHERE 1 ".$sPrizeWhere." GROUP BY P.`lvtopid` ", 43200 );
			$aResult    = array();
			foreach( $aPrizeTemp as $Temp )
			{
			    $aResult[$Temp["lvtopid"]]["prize"] = $Temp["SUM_PRIZE"];
				$aResult[$Temp["lvtopid"]]["bonus"] = 0.00;
				$aResult[$Temp["lvtopid"]]["point"] = 0.00;
			}
		    //获取中奖金额
			$sBonusField = " SUM(P.`bonus`) AS `SUM_BONUS`,P.`lvtopid` ";
			$sBonusConditon = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    {
		       $sBonusConditon .= " AND P.`bonustime`>= '" . $sQueryStartTime ."'";  
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		    {
		        $sBonusConditon .= " AND P.`bonustime`<= '" . $sQueryEndTime ."'";
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		    {
		        $sBonusConditon .= " AND (P.`bonustime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		    }
			$sBonusWhere = " AND P.`iscancel` = '0' AND a.`userid`=P.`userid` " . $sCondition . $sBonusConditon . $sUserCondition;	
			$aBonusTemp = $this->oDB->getDataCached( " SELECT ".$sBonusField." FROM ".$sTableName." WHERE 1 ".$sBonusWhere." GROUP BY P.`lvtopid` ", 43200 );
			foreach( $aBonusTemp as $Temp )
			{
				$aResult[$Temp["lvtopid"]]["bonus"] = $Temp["SUM_BONUS"];
				$aResult[$Temp["lvtopid"]]["prize"] = isset($aResult[$Temp["lvtopid"]]["prize"]) ? $aResult[$Temp["lvtopid"]]["prize"] : 0.00;
				$aResult[$Temp["lvtopid"]]["point"] = 0.00;
			}
			//获取总返点
			/**
			 * 如:在2010-05-18 02:20:00到2010-05-19 02:20:00时间内进行查看 
			 * 返点计算方式:
			 *  1.在2010-05-18 02:20:00到2010-05-19 02:20:00时间的正常单子进行的返点值[加上].
			 *  2.在2010-05-18 02:20:00之前购买的单子,在2010-05-18 02:20:00到2010-05-19 02:20:00进行撤单的返点值.[减去]
			 * 	3.在在2010-05-18 02:20:00到2010-05-19 02:20:00购买的单子,在2010-05-19 02:20:00以后进行撤单的返点值.[加上]
			 */
			$sPointTableName = " `projects` AS P JOIN `userdiffpoints` AS UDP JOIN `usertree` AS a ";
			$sPointField = " SUM(UDP.diffmoney) AS `SUM_POINT`,P.`lvtopid` ";
			$sPointConditon = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    {
		       $sPointConditon .= " AND UDP.`sendtime`>= '" . $sQueryStartTime ."'";  
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		    {
		        $sPointConditon .= " AND UDP.`sendtime`<= '" . $sQueryEndTime ."'";
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		    {
		        $sPointConditon .= " AND (UDP.`sendtime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		    }
		    $sPointConditon .= " AND UDP.`projectid`=P.`projectid` AND UDP.`status`=1";
			//在指定时间内正常单子进行的返点值
		    $sPointWhere = " AND P.`iscancel` = '0' AND a.`userid`=P.`userid` " . $sCondition . $sPointConditon . $sUserCondition;	
			$aPointTemp = $this->oDB->getAll( " SELECT ".$sPointField." FROM ".$sPointTableName." WHERE 1 ".$sPointWhere." GROUP BY P.`lvtopid` " );
			//在前一天购买的单子，在当天进行撤单的返点值[减去]
			$sPointPlusConditon = " AND P.`writetime`<'".$sQueryStartTime."' AND P.`canceltime` > '".$sQueryStartTime."'";
			$sPointPlusConditon .= " AND UDP.`projectid`=P.`projectid` AND UDP.`sendtime` != '0000-00-00 00:00:00'";
			$sPointWherePlus = " AND P.`iscancel` > '0' AND a.`userid`=P.`userid` " . $sCondition . $sPointPlusConditon . $sUserCondition;	
			$aPointTempPlus = $this->oDB->getDataCached( " SELECT ".$sPointField." FROM ".$sPointTableName." WHERE 1 ".$sPointWherePlus." GROUP BY P.`lvtopid` ", 43200 );
			$aPointPlusResult = array();
			foreach ( $aPointTempPlus as $aPointPlus )
			{
				$aPointPlusResult[$aPointPlus['lvtopid']] = $aPointPlus;
			}
			//在指定时间内购买的单子，在指定时间之后进行撤单的返点值[加上]
			$sPointReduceConditon = " AND P.`canceltime` > '".$sQueryEndTime."'";
			$sPointReduceConditon .= str_replace("UDP.`sendtime`","P.`writetime`",$sPointConditon);
			$sPointReduceConditon = str_replace("UDP.`status`=1","UDP.`sendtime` != '0000-00-00 00:00:00'",$sPointReduceConditon);
			$sPointWhereReduce = " AND P.`iscancel` > '0' AND a.`userid`=P.`userid` " . $sCondition . $sPointReduceConditon . $sUserCondition;	
			$aPointTempReduce = $this->oDB->getDataCached( " SELECT ".$sPointField." FROM ".$sPointTableName." WHERE 1 ".$sPointWhereReduce." GROUP BY P.`lvtopid` ", 43200 );
			$aPointReduceResult = array();
			foreach ( $aPointTempReduce as $aPointReduce )
			{
			    $aPointReduceResult[$aPointReduce['lvtopid']] = $aPointReduce;
			}
			foreach( $aPointTemp as $Temp )
			{
			    $aResult[$Temp["lvtopid"]]["bonus"] = isset($aResult[$Temp["lvtopid"]]["bonus"]) ? $aResult[$Temp["lvtopid"]]["bonus"] : 0.00;
			    $aResult[$Temp["lvtopid"]]["prize"] = isset($aResult[$Temp["lvtopid"]]["prize"]) ? $aResult[$Temp["lvtopid"]]["prize"] : 0.00;
			    $aResult[$Temp["lvtopid"]]["point"] = $Temp["SUM_POINT"];
			}
			//在前一天购买的单子，在当天进行撤单的返点值[减去]
			foreach ( $aPointPlusResult as $ilovtopid => $aPointPlus )
			{
			    if(!isset($aResult[$ilovtopid]["point"]))
			    {
			        $aResult[$ilovtopid]["bonus"] = isset($aResult[$ilovtopid]["bonus"]) ? $aResult[$ilovtopid]["bonus"] : 0.00;
			        $aResult[$ilovtopid]["prize"] = isset($aResult[$ilovtopid]["prize"]) ? $aResult[$ilovtopid]["prize"] : 0.00;
			        $aResult[$ilovtopid]["point"] = -$aPointPlusResult[$ilovtopid]['SUM_POINT'];
			    }
			    else 
			    {
			        $aResult[$ilovtopid]["point"] -= $aPointPlusResult[$ilovtopid]['SUM_POINT'];
			...
			//在指定时间内购买的单子，在指定时间之后进行撤单的返点值[加上]
			foreach ( $aPointReduceResult as $ilovtopid => $aPointReduce )
			{
			    if(!isset($aResult[$ilovtopid]["point"]))
			    {
			        $aResult[$ilovtopid]["bonus"] = isset($aResult[$ilovtopid]["bonus"]) ? $aResult[$ilovtopid]["bonus"] : 0.00;
			        $aResult[$ilovtopid]["prize"] = isset($aResult[$ilovtopid]["prize"]) ? $aResult[$ilovtopid]["prize"] : 0.00;
			        $aResult[$ilovtopid]["point"] = $aPointReduceResult[$ilovtopid]['SUM_POINT'];
			    }
			    else
			    {
			        $aResult[$ilovtopid]["point"] += $aPointReduceResult[$ilovtopid]['SUM_POINT'];
			...
			/*计算大额撤单手续费*/
			$iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
			$sCanelFreeCondition = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    {
		       $sCanelFreeCondition .= " AND P.`writetime`>= '" . $sQueryStartTime ."'";  
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		    {
		        $sCanelFreeCondition .= " AND P.`writetime`<= '" . $sQueryEndTime ."'";
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		    {
		        $sCanelFreeCondition .= " AND (P.`writetime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		    }
			$sTableName = "`orders` AS o LEFT JOIN `usertree` AS a ON(o.`fromuserid`=a.`userid`)
                 LEFT JOIN `projects` as P ON (P.`projectid` = o.`projectid`)";
			$sField = "SUM(o.`amount`) AS amount,P.`lvtopid`";
			$sWhere = " AND o.`ordertypeid` = '$iOrderTypeId'".$sCondition.$sCanelFreeCondition.$sUserCondition;
			$aTempAmout = $this->oDB->getDataCached(" SELECT ".$sField." FROM ".$sTableName." WHERE 1 ".$sWhere. " GROUP BY P.`lvtopid` ", 43200 );
			foreach ($aTempAmout as $aAmount)
			{
			    $aResult[$aAmount['lvtopid']]['prize'] += $aAmount['amount'];
			}
		...
		{ // 查询非总代的
			...
			$sPrizeField = " SUM(P.`totalprice`) AS `SUM_PRIZE`,"
			             ." SUBSTRING_INDEX(a.`parenttree`,',',".$iUserLevel.") AS `USERTREE`,"
			             ." P.`userid`,a.`username`,a.`usertype`,a.`parentid` ";
			$sWhere     = " AND P.`iscancel` = '0' AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." a.`userid` = P.`userid` ".$sCondition.$sPrizeConditon;
			$aTemp1     = $this->oDB->getDataCached(" SELECT ".$sPrizeField." FROM ".$sTableName." where 1 ".$sWhere.
			                                  $sUserCondition." AND P.`iscancel`='0' group by P.`userid`", 43200 );	
			$aResult    = array();
			foreach( $aTemp1 as $v)
			{				
				$a = explode( ",", $v["USERTREE"] );
				...
				{
					$aResult[$sStrUser]["prize"] = $v["SUM_PRIZE"];
					$aResult[$sStrUser]["bonus"] = 0; 
					$aResult[$sStrUser]["point"] = 0; //防止返点值为0
				}
			}
			//获取中奖金额
			$sBounsConditon = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    {
		       $sBounsConditon .= " AND P.`bonustime`>= '" . $sQueryStartTime ."'";  
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		    {
		        $sBounsConditon .= " AND P.`bonustime`<= '" . $sQueryEndTime ."'";
		    }
		    if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		    {
		        $sBounsConditon .= " AND (P.`bonustime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		    }
			$sBonusField = " SUM(P.`bonus`) AS `SUM_BONUS`,"
			             ." SUBSTRING_INDEX(a.`parenttree`,',',".$iUserLevel.") AS `USERTREE`,"
			             ." P.`userid`,a.`username`,a.`usertype`,a.`parentid` ";
			$sWhere     = " AND P.`iscancel` = '0' AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." a.`userid` = P.`userid` ".$sCondition.$sBounsConditon;
			$aTemp1     = $this->oDB->getDataCached(" SELECT ".$sBonusField." FROM ".$sTableName." where 1 ".$sWhere.
			                                  $sUserCondition." AND P.`iscancel`='0' group by P.`userid`", 43200 );	
			foreach( $aTemp1 as $v)
			{				
				$a = explode( ",", $v["USERTREE"] );
				...
				{
					$aResult[$sStrUser]["bonus"] = $v["SUM_BONUS"];
					$aResult[$sStrUser]["prize"] = isset($aResult[$sStrUser]["prize"]) ? $aResult[$sStrUser]["prize"] : 0.00;
				}
			}
			/*计算大额撤单手续费*/
			$iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
			...
			$sField     = " Sum(o.`amount`) AS `SUM_AMOUNT`, a.`userid`,"
			             ." SUBSTRING_INDEX(a.`parenttree`,',',".$iUserLevel.") AS `USERTREE`,a.`parentid`";
			$sWhere     = " AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." a.`userid` = P.`userid` AND o.`ordertypeid` = '$iOrderTypeId' ".$sCondition.$sCanelFreeCondition;
			$aTemp1     = $this->oDB->getDataCached(" SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhere.
			                                  $sUserCondition." GROUP BY o.`fromuserid`", 43200 );
			foreach( $aTemp1 as $v)
			{				
				$a = explode( ",", $v["USERTREE"] );
				if( $v["parentid"] == $iUserId )
			...
			$sWhere     = " AND P.`iscancel` = '0' AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." UDP.`projectid`=P.`projectid` AND a.`userid` = UDP.`userid` ".$sCondition;
			//在前一天购买的单子，在当天进行撤单的返点值[减去]
			$sPointReduceConditon = " AND P.`writetime`<'".$sQueryStartTime."' AND P.`canceltime` > '".$sQueryStartTime."'";
			$sPointReduceConditon .= " AND UDP.`sendtime` != '0000-00-00 00:00:00'";
			$sWhereReduce = " AND P.`iscancel` > '0' AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." UDP.`projectid`=P.`projectid` AND a.`userid` = UDP.`userid` ".$sPointReduceConditon.$sCondition;
			//在指定时间内购买的单子，在指定时间之后进行撤单的返点值[加上]
			$sPointPlusConditon = " AND P.`canceltime` > '".$sQueryEndTime."'";
			$sPointPlusConditon .= str_replace("UDP.`sendtime`","P.`writetime`",$sPointConditon);
			$sWherePlus	  = " AND P.`iscancel` > '0' AND find_in_set(".$iUserId.",a.`parenttree`) AND "
			             ." UDP.`projectid`=P.`projectid` AND a.`userid` = UDP.`userid` ".$sPointPlusConditon.$sCondition;
			$aTemp       = $this->oDB->getDataCached("SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhere.$sPointConditon.
			               $sUserCondition." AND UDP.`status`=1 AND P.`iscancel`='0' GROUP BY UDP.`userid`", 43200 );
			$aTempReduce = $this->oDB->getDataCached("SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWhereReduce.
			               $sUserCondition." GROUP BY UDP.`userid`", 43200 );
			$aTempPlus   = $this->oDB->getDataCached("SELECT ".$sField." FROM ".$sTableName." where 1 ".$sWherePlus.
			               $sUserCondition." GROUP BY UDP.`userid`", 43200 );
			//说明:如果需要及时计算，请取消 AND UDP.`status`='1'反之加上这个条件//AND UDP.`status`='1'	
			foreach( $aTemp as $v)
			{				
				...
				{
					$aResult[$sStrUser]["point"] =  $v["SUM_POINT"];
					$aResult[$sStrUser]["bonus"] = isset($aResult[$sStrUser]["bonus"]) ? $aResult[$sStrUser]["bonus"] : 0.00; 
					$aResult[$sStrUser]["prize"] = isset($aResult[$sStrUser]["prize"]) ? $aResult[$sStrUser]["prize"] : 0.00; 
				}
				unset($a);
			}
			unset($aTemp);
			//在前两天购买的单子，在前一天进行撤单的返点值[减去]
			foreach( $aTempReduce as $v)
			{				
				$a = explode( ",", $v["USERTREE"] );
				if( $v["parentid"] == $iUserId )
				{
	...
	/**
	 * 后台查询指定用户的因为自身获取到的相关的返点和销售总额
	 *
	 * @param integer $iUserId
	 * @param string $sCondition
	 * @author mark
	 */
	function getAdminUserBonusAndPrize( $iUserId, $sCondition, $sQueryStartTime = '', $sQueryEndTime = '' )
	{
		$aResult = array();
		$iUserId = intval($iUserId);
		if( $iUserId <= 0 )
		...
		elseif ($iUserId == $aUser['lvtopid'])//总代：统计返点
		{
		    $sSql = " SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		          FROM `usertree` AS UT,`userdiffpoints` AS UDP
		          LEFT JOIN `projects` AS P ON (P.`projectid`=UDP.`projectid`) 
		          WHERE 1 " . $sCondition.$sPointConditon. " AND P.`iscancel`='0' AND UDP.`userid`='".$iUserId."' AND UT.`userid`='".$iUserId."' 
		          GROUP BY P.`modes`";
		    $aResult = $this->oDB->getDataCached( $sSql, 43200 );
		}
		else //非总代
		{
		    //获取自身的总代购费
		    ...
		    $sSql = " SELECT SUM(P.`totalprice`) AS `price`,P.`modes`,
		          UT.`username`,UT.`usertype` FROM `usertree` AS UT LEFT JOIN `projects` AS P ON 
		          (P.`userid`=UT.`userid`) WHERE 1 "
		    .$sCondition.$sPrizeConditon." AND P.`iscancel`='0' AND P.`userid`='".$iUserId."' GROUP BY P.`modes`";
		    $iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
		    $aResult = $this->oDB->getDataCached( $sSql, 43200 );
		    //获取自身中奖金额
			$sBounsConditon = ' AND 1 ';
		    ...
		    $sSql = " SELECT SUM(P.`bonus`) AS `bonus`,P.`modes`,
		          UT.`username`,UT.`usertype` FROM `usertree` AS UT LEFT JOIN `projects` AS P ON 
		          (P.`userid`=UT.`userid`) WHERE 1 "
		    .$sCondition.$sBounsConditon." AND P.`iscancel`='0' AND P.`userid`='".$iUserId."' GROUP BY P.`modes`";
		    $iOrderTypeId = ORDER_TYPE_CDSXF;//撤单手续费账变类型ID
		    $aBonusResult = $this->oDB->getDataCached( $sSql, 43200 );
		    //自身返点
		    $sSql = " SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		              FROM `usertree` AS UT,`userdiffpoints` AS UDP
		              LEFT JOIN `projects` AS P ON (P.`projectid`=UDP.`projectid`) 
		              WHERE 1 " . $sCondition.$sPointConditon. " AND P.`iscancel`='0' AND UDP.`userid`='".$iUserId."' AND UT.`userid`='".$iUserId."' 
		              GROUP BY P.`modes`";
		    $aPointResult = $this->oDB->getDataCached( $sSql, 43200 );
		    //在前一天购买的单子，在当天进行撤单的返点值[减去]
			$sPointReduceConditon = " AND P.`writetime`<'".$sQueryStartTime."' AND P.`canceltime` > '".$sQueryStartTime."'";
			$sPointReduceConditon .= " AND UDP.`sendtime` != '0000-00-00 00:00:00'";
			$sWhereReduce = " AND P.`iscancel` > '0' AND UDP.`userid`='".$iUserId."' AND UT.`userid`='".$iUserId."'".$sPointReduceConditon.$sCondition;
			$aPointReduce = $this->oDB->getDataCached(" SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		              			FROM `usertree` AS UT,`userdiffpoints` AS UDP
		              			LEFT JOIN `projects` AS P ON (P.`projectid`=UDP.`projectid`)  WHERE 1 ".$sWhereReduce.
			               		$sCondition." GROUP BY P.`modes`");
			//在指定时间内购买的单子，在指定时间之后进行撤单的返点值[加上]
			$sPointPlusConditon = " AND P.`canceltime` > '".$sQueryEndTime."'";
			$sPointPlusConditon .= str_replace("UDP.`sendtime`","P.`writetime`",$sPointConditon);
			$sWherePlus	  = " AND P.`iscancel` > '0' AND UDP.`userid`='".$iUserId."'  AND UT.`userid`='".$iUserId."'".$sPointPlusConditon.$sCondition;
			$aPointPlus   = $this->oDB->getAll(" SELECT SUM(UDP.`diffmoney`) AS `point`,P.`modes`, UT.`username`,UT.`usertype`
		              			FROM `usertree` AS UT,`userdiffpoints` AS UDP
		              			LEFT JOIN `projects` AS P ON (P.`projectid`=UDP.`projectid`)  WHERE 1 ".$sWherePlus.
			               		$sCondition." GROUP BY P.`modes`");
		    //计算大额撤单手续费
		    $sCanelFreeCondition = ' AND 1 ';
			if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		    ...
		    $sSql = "SELECT SUM(`amount`) AS amount ,P.`modes` FROM `orders` AS o LEFT JOIN `projects` AS P ON( o.`projectid` = P.`projectid` )
		         WHERE `fromuserid` = '$iUserId' AND o.`ordertypeid` = '$iOrderTypeId'" . $sCondition .$sCanelFreeCondition." GROUP BY P.`modes`";
		    $aPrice = $this->oDB->getDataCached( $sSql, 43200 );//获取大额撤单手续费
		}
		$aLasResult = array();
		$aLasResult['bonus']  = 0.00;
		$aLasResult['point']  = 0.00;
		...
		//在前一天购买的单子，在当天进行撤单的返点值[减去]
		if( !empty($aPointReduce) )
		{
		    foreach ( $aPointReduce as $aValue )
		    {
		        $aLasResult['point'] -= isset($aValue['point']) ? $aValue['point'] : 0;
		    }
		}
		...
		if( !empty($aPointPlus) )
		{
		    foreach ( $aPointPlus as $aValue )
		    {
		        $aLasResult['point'] += isset($aValue['point']) ? $aValue['point'] : 0;
		    }
		}
		if( !empty($aBonusResult) )
		{
		    foreach ( $aBonusResult as $aValue )
		    {
		        $aLasResult['bonus'] += isset($aValue['bonus']) ? $aValue['bonus'] : 0;
		    }
		}
	 ...
	 * @return array
	 */
	function getAdminTotalUserBonusByMethod( $iUserId, $sCondition, $needCheck = TRUE, $sQueryStartTime = '', $sQueryEndTime = '' )
	{
		if( !isset($needCheck) )
		{
			$needCheck = TRUE;
		...
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(UDP.`diffmoney`) AS `summoney`,P.`modes`"
			." FROM `userdiffpoints` AS UDP"
			." JOIN `projects` AS P JOIN `usertree` AS UT"
			." WHERE UDP.`projectid` = P.`projectid` AND UT.`userid`=UDP.`userid`".$sPointConditon
			." AND (FIND_IN_SET(".$iUserId.",UT.`parenttree`) OR UT.`userid`='".$iUserId."')"
			." AND P.`iscancel`='0'"
			." GROUP BY P.`lotteryid`,P.`methodid`,P.`modes`";
		$aResult[1] = $this->oDB->getDataCached( $sSql, 43200 );
		//在前一天购买的单子，在当天进行撤单的返点值[减去]
		$sPointPlusConditon = " AND P.`writetime`<'".$sQueryStartTime."' AND P.`canceltime` > '".$sQueryStartTime."'";
		$sPointPlusConditon = $sPointConditon.$sPointPlusConditon;
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(UDP.`diffmoney`) AS `summoney`,P.`modes`"
			." FROM `userdiffpoints` AS UDP"
			." JOIN `projects` AS P JOIN `usertree` AS UT"
			." WHERE UDP.`projectid` = P.`projectid` AND UT.`userid`=UDP.`userid`".$sPointPlusConditon
			." AND (FIND_IN_SET(".$iUserId.",UT.`parenttree`) OR UT.`userid`='".$iUserId."')"
			." AND P.`iscancel`>'0'"
			." GROUP BY P.`lotteryid`,P.`methodid`,P.`modes`";
		$aResult[4] = $this->oDB->getDataCached( $sSql, 43200 );
		//在指定时间内购买的单子，在指定时间之后进行撤单的返点值[加上]
		$sPointReduceConditon = " AND P.`canceltime` > '".$sQueryEndTime."'";
		$sPointReduceConditon = $sPointConditon.$sPointReduceConditon;
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(UDP.`diffmoney`) AS `summoney`,P.`modes`"
			." FROM `userdiffpoints` AS UDP"
			." JOIN `projects` AS P JOIN `usertree` AS UT"
			." WHERE UDP.`projectid` = P.`projectid` AND UT.`userid`=UDP.`userid`".$sPointReduceConditon
			." AND (FIND_IN_SET(".$iUserId.",UT.`parenttree`) OR UT.`userid`='".$iUserId."')"
			." AND P.`iscancel`>'0'"
			." GROUP BY P.`lotteryid`,P.`methodid`,P.`modes`";
		$aResult[5] = $this->oDB->getDataCached( $sSql, 43200 );
		//获取奖金
		$sBonusCondition = ' AND 1 ';
		if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		{
		    $sBonusCondition .= " AND P.`bonustime`>= '" . $sQueryStartTime ."'";
		}
		if( $sQueryEndTime != '' && $sQueryStartTime == '' )
		{
		    $sBonusCondition .= " AND P.`bonustime`<= '" . $sQueryEndTime ."'";
		}
		if( $sQueryEndTime != '' && $sQueryStartTime != '' )
		{
		    $sBonusCondition .= " AND (P.`bonustime` BETWEEN '" . $sQueryStartTime . "' AND '" . $sQueryEndTime. "' ) ";
		}
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(P.`bonus`) AS `sumbonus`,P.`modes`"
			." FROM `projects` AS P JOIN `usertree` AS UT"
			." WHERE UT.`userid`=P.`userid`".$sBonusCondition
			." AND (FIND_IN_SET(".$iUserId.",UT.`parenttree`) OR UT.`userid`='".$iUserId."')"
			." AND P.`iscancel`='0'"
			." GROUP BY P.`lotteryid`,P.`methodid`,P.`modes`";
		$aResult[2] =  $this->oDB->getDataCached( $sSql, 43200 );
		//获取投注量
		$sPrizeConditon = ' AND 1 ';
		if( $sQueryEndTime == '' && $sQueryStartTime != '' )
		{
		...
		$sSql = "SELECT P.`lotteryid`,P.`methodid`,SUM(P.`totalprice`) AS `sumprice`,P.`modes`"
			." FROM `projects` AS P JOIN `usertree` AS UT"
			." WHERE UT.`userid`=P.`userid`".$sPrizeConditon
			." AND (FIND_IN_SET(".$iUserId.",UT.`parenttree`) OR UT.`userid`='".$iUserId."')"
			." AND P.`iscancel`='0'"
			." GROUP BY P.`lotteryid`,P.`methodid`,P.`modes`";
		$aResult[3] =  $this->oDB->getDataCached( $sSql, 43200 );
		return $aResult;
	}
}
?>
