...
class controller_security extends basecontroller 
{
    ...
    function actionTran()
    {
	    ...
	    { // 处理转账过程
            ...
            $aUserinfo = $oUserFund->getFundByUser( intval($_SESSION['userid']), $sFields );
            if( empty($aUserinfo) )
            {
	    ...
	    { // 显示转账界面
    		...
    		{
    			$this->actionCheckPass('security', 'tran');
    			EXIT;
    		}
            */
            ...
            $aTranfer = array();
            $maxPickUpMoney = 0.00;
            $aTranfer['iUserId'] = $_SESSION['userid'];
    	    ...
    	    {
    	        $maxPickUpMoney = floor($mAnswers['data']*100) / 100;
    	    }
    ...
    public function actionPickUp(){
    	if( !empty($_POST['flag']) && $_POST['flag'] == 'doTranfer' ){ // 处理转账过程
            ...
            // 检查目标平台是否有足够的资金
	        $aUserInfo = array();
	        $aUserInfo['iUserId'] = $_SESSION['userid'];
		    ...
		    $fMoney = floor($_POST['fmoney']*100) / 100;
		    $mAnswers['data'] = floor($mAnswers['data']*100) / 100;
		    if ( $fMoney <= 0 ){
		    	sysMsg( "转账相关数据不符合规范", 2 );
		    }
		    if ( $fMoney > $mAnswers['data'] ){
