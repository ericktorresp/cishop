...
class model_usersession extends basemodel 
{
	...
	public function checkMenuAccess( $iUserId = 0, $sControlName = '', $sActionName = '' )
	{
	    ...
	    //08:获取用户信息
        $sSql = " SELECT ut.`userid`,ut.`username`,ut.`usertype`,ut.`parentid`,ugs.`groupid`,ugs.`extendmenustr` 
        ...
        if( $aUserData['parentid'] == 0 || $aUserData['usertype'] == 2 )
        {//总代和总代管理员不允许使用
            return -9;
        }
        //05: 检测用户是否被冻结
...
//            }
		...
		$sSql  = "SELECT `title`,`menuid`,`actionlog`,`ismenu`,`islink`,`islabel` FROM `usermenu` " 
				  ...
				  . "' AND `actioner`='" . daddslashes($sActionName) . "' AND `isdisabled`='0' AND `menutype`='0' ";
		...
		{
			$sSql = "SELECT `menustrs` FROM `proxygroup` 
					 WHERE `groupid`='".$aUserData['groupid']."' AND `isdisabled`='0' ";
		}
