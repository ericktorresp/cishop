...
{   
    ...
    protected function authCheck()
    {
        ...
        /*TODO:高低频并行*/
        if( empty($_SESSION['userid']) && !empty($_COOKIE['iUserId']) 
            && !empty($_COOKIE['sUserName']) && !empty($_COOKIE['sPassword']) )
        {//到银行大厅去同步登陆然后跳转回来
        	header("location: /index.php?controller=default&tzurl=yx");
        }
        /**并行完*/
        /*********************权限检测****************/
        ...
        {//未登陆
            ...
            $aLinks = array( array('url'=>'../index.php?controller=default&action=login') );
        ...
        elseif( $result === -3 )
        {
            sysMsg( "频道已关闭或没有激活", 2, "", "top", FALSE );
        }
        elseif( $result === -4 )
        {
            sysMsg( "没有操作权限", 2, "", "top", FALSE );
        }
        elseif( $result === -5 )
        {//菜单未注册
        ...
        elseif( $result === -7 )
        {
            sysMsg( "用户组被禁用或者不存在<br />请与管理员联系", 2, "", "top", FALSE );
        }
        elseif( $result === -9 )
        {
            sysMsg( "没有操作权限", 2, "", "top", FALSE );
