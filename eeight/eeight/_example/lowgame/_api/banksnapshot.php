...
class api_banksnapshot extends baseapi
{
    public function _runProcess()
    {
    	...
    	{ //生成对象，整理对象
    		$oBankSnapShot = new model_snapshot();
    		/**
    		 * 帐号类型[是否为tester,冻结状态]
    		 */
    	...
    	foreach( $aData as $iUserId => $data )
    	{
            ...
            foreach($aTemp as $v)
            {
                ...
                if( isset( $data[$i][$t]) )
                {
                    ...
                    $aResult[$iUserId]["totalbuy"]    += $data[$i][$t]["totalbuy"];
                    $aResult[$iUserId]["totalpoint"]  += $data[$i][$t]["totalpoint"];
                    $aResult[$iUserId]["totalbingo"]  += $data[$i][$t]["totalbingo"];
                    $aResult[$iUserId]["totalbalance"]+= $data[$i][$t]["totalbalance"];
                    ...
                    $aResult[$iUserId]["ta"]          += $data[$i][$t]["tc"];
