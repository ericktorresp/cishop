...
$.alert = function(opts){   //模拟警告窗口
    ...
    html += '<a href="javascript:" id="james_dialogUI_alert_close" class="'+opts.closeCss+'"><em style="display:none;">[X]</em></a></td></tr>';
    html += '<tr><td><div id="james_dialogUI_alert_content" class="'+opts.contentCss+'">'+opts.message+'</div></td></tr></table></div>';
    $html = $(html);
    $.blockUI({
        message : $html,
...
$.confirm = function(opts){//模拟询问窗口
    ...
    html += '<div style="padding:0;padding-left:5px;font-size:13px;text-transform:capitalize;font-weight:bold;color:#FFF;float:left;height:20px;line-height:20px;">'+opts.title+'</div>';
    html += '<a href="javascript:" id="james_dialogUI_confirm_close" class="'+opts.closeCss+'"><em style="display:none;">[X]</em></a></td></tr>';
    html += '<tr><td><div id="james_dialogUI_confirm_content" class="'+opts.contentCss+'"><p style="margin:0px;padding:5px;">'+opts.message+'</p><center><button class="'+opts.buttonCss+'" id="james_dialogUI_confirm_yes">'+opts.textyes+'</button>&nbsp;<button id="james_dialogUI_confirm_no" class="'+opts.buttonCss+'">'+opts.textno+'</button></center></div></td></tr></table></div>';
    $html = $(html);
    $.blockUI({
        message : $html,
        css: {width : '300px','max-width':'400px',border: 0},
        overlayCSS:{backgroundColor: '#FFFFFF',opacity:0.6,cursor:'default'}
    });
    ...
    $("#james_dialogUI_confirm").Drags({handler: '#james_dialogUI_confirm_title'});
    //return isyes;
}
$.confirm.defaults = {
    message    : null,     //询问内容
...
$.ajaxUploadUI = function(opts){    //文件上传/载入文件
    ...
    opts.fadeOut = 0;
    opts.message = '<form action="'+opts.url+'" method="POST" name="james_uploadUI" target="james_uploadUI_iframe" id="james_uploadUI" enctype="multipart/form-data" style="margin:0;padding:5px 0 10px 0;">'+opts.message+'<input type="file" size="20" name="'+opts.inputfile+'" id="james_uploadUI_file" value="选择文件">&nbsp;&nbsp;<button type="button" id="james_uploadUI_submit" class="'+opts.button.css+'" disabled>'+opts.button.text+'</button></form><span style="display:none;text-align:center;" id="james_uploadUI_loading"><center>'+opts.loadtext+'<br /><img src="'+opts.loadimg+'" align="absmiddle"></center></span><span id="james_uploadUI_error" style="color:red;"></span><iframe name="james_uploadUI_iframe" id="james_uploadUI_iframe" style="width:0px; height:0px;display:none;"></iframe>';
    $.alert(opts);
		...
		if( xml || isTimeout == "timeout"){	//如果返回数据或者请求超时
            ...
            try{
                ...
                if( status != "error" ){// process the data (runs the xml through httpData regardless of callback)
                    ...
                    $("<center><button id='james_uploadUI_finish' class='"+opts.button.css+"'>完成</button></center>").appendTo("#james_uploadUI_error");
    ...
    $("#james_uploadUI_submit").click(function(){
        ...
        if( filepath != "" && filepath != null  ){//选择了文件
            ...
            if( $.inArray(filetype,opts.filetype) == -1  ){
                showError(filetype+"文件非法，允许文件格式为："+opts.filetype.join("、"));
                return false;
            }
            uploadStart();
            if( s.timeout > 0 ){
