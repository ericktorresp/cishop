    ...
    $.fn.jamesDrag = function(opts) {
        ...
        var dragndrop = {
            drag: function(e) {
                ...
                var leftpx = dragData.left + e.pageX - dragData.offLeft;
                var toppx = dragData.top + e.pageY - dragData.offTop;
                var windowheight = ($(window).height()-305)/2;
                if(toppx < -windowheight){
                    toppx = -windowheight;
                }
                if(toppx > windowheight){
                    toppx = windowheight;
                }
                var windowleft = ($(window).width()-300)/2;
                var windowright = windowleft-3;
                if(leftpx < -windowleft){
                    leftpx = -windowleft;
                }
                if(leftpx > windowright){
                    leftpx = windowright;
                }
                dragData.target.css({
                    left: leftpx,
