...
APE.Home = new Class({
	...
	initialize: function(){
		...
		this.els = {};
		this.msn_user = {ape:$('first_tree')};
		this.twitter_user = {ape:$('second_tree')};
		...
		this.addEvent('load', function() { 
			this.core.start({"home": true, "name":this.rand_chars()}) 
		});
	...
	show_tweet:function(tweet)
	{
		...
		(function(){
		 	if (Browser.Engine.trident) {
				new Fx.Morph(els.real_cnt, {duration: 2000}).start({'opacity':0});
			}
			var mtop = this.rand(30, 90);
			new Fx.Morph(els.el,{transition:Fx.Transitions.linear,duration:2000}).start({
	...
	raw_butterfly: function(res){
		this.write_message(decodeURIComponent(res.data.user + ' : ' + res.data.text),this.msn_user,false,{'style':'msg_msn'});
	},
	write_message: function(message,sender,no_hide,options){
		...
		var msg = new Element('div',{'class':'ape_message_container ' + options.style});
		if (Browser.Engine.trident &amp;&amp; !document.querySelectorAll) msg.setStyle('margin-top',0);
		var left = new Element('div',{'class':'msg_left'}).inject(msg);
		var cnt = new Element('div',{'class':'msg'}).inject(left);
		message = decodeURIComponent(message);
		var right = new Element('div',{'class':'msg_right','text':message.substring(0,this.max_txt)}).inject(cnt);
		...
		//Show message
		new Fx.Morph(cnt,{'duration':200}).start({'opacity':1});
		...
		return {'el':msg,'cnt':right, 'real_cnt':cnt, 'left':left};
	},
	...
	add_chat:function(){
		...
		this.msn_talk = new Element('div',{'styles':{'opacity':0},'html':msn_talk,'class':'msn_talk'}).inject(this.msn_user.ape);

		this.msn_user.ape.addEvent('mouseover',function(){
			this.msn_talk.morph({'opacity':1});
		}.bind(this));
		(function(){
			this.write_message(msn_msg,this.msn_user,false,{'style':'msg_msn'});
		}.delay(1500,this));
		this.msn_user.ape.addEvent('mouseout',function(){
			this.msn_talk.morph({'opacity':0});
		}.bind(this));

		this.els.chat_box_home = new Element('div',{'class':'chat_box_home'}).inject($('master_container'),'before').morph({height:16});
		//this.els.chat_box_home.store('fx',new Fx.Morph(this.els.chat_box_home));
		form = new Element('form').inject(this.els.chat_box_home);
		this.els.input = new Element('input',{'type':'text','value':txt_input,'class':'text autoclear'}).inject(form);
		...
		new Element('input',{'type':'submit','class':'submit','value':''}).inject(form);
	...
	create_ape: function(user, pipe)
	{
		if(pipe.name.contains('demohome'))
		{
			...
			var fx = new Fx.Morph(user.ape_container,{duration:4000, transition: Fx.Transitions.linear});
	...
	set_houba:function(user){
		...
		user.ape_anim.start( [
			{'start':3,'end':4,'delay':0,'repeat':1},
			{'start':1,'end':1,'repeat':0,'delay':0},
		]);
	},
	set_idle: function(user){
		...
		this.reset_user(user);
		user.ape_anim.start([
			{'start':1,'end':1,'repeat':0,'delay':this.rand(0,1000)},
			{'start':1,'end':2,'delay':0,'repeat':this.rand(3,5)},
			{'start':1,'end':1,'delay':2500,'repeat':0},
			{'start':3,'end':4,'delay':0,'repeat':1},
			{'start':1,'end':1,'repeat':0,'delay':3000}
		]);
	},
	next_link: function(el){
		new Element('a',{'text':msg_next}).inject(el.cnt);
	...
	initialize: function(selector, options) {
		...
		tips.addEvents({
			'mouseenter': function(ev) {
				...
				this.els.tip.setStyles({'left': elx, 'top':ely});
			}.bind(this),
			'mouseleave': function() {
