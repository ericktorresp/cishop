...
class UserDepositFormWizard(FormWizard):
    ...
    def get_form(self, step, data=None):
        form = super(UserDepositFormWizard, self).get_form(step, data)
        if step == 1 and data:
            method = DepositMethod.objects.get(id__exact=data.get('0-deposit_method'))
            form.fields['amount'] = forms.DecimalField(label=_('amount'), max_digits=7, decimal_places=2, max_value = method.max_deposit, min_value = method.min_deposit)
        return form
